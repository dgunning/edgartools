...
``SecurityHolding`` and ``TransactionActivity`` are the per-row records; the
``OwnershipSummary`` hierarchy (``InitialOwnershipSummary`` for Form 3,
``TransactionSummary`` for Forms 4/5) provides DataFrame export and rich display
of an entire filing's holdings or transactions.
"""
