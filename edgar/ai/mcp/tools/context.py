...
Takes an accession number or SEC URL and returns the filing's AI context.
If the filing has a data object (TenK, Form4, ThirteenF, etc.), returns
that object's to_context() output. Otherwise returns the Filing's to_context().

This is the fundamental "tell me about this filing" tool.
"""
...
@tool(
    name="edgar_context",
    description="""Get AI-optimized context for any SEC filing by accession number or URL.

Returns structured context about the filing: what it is, key data, and available actions.
If the filing has a structured data object (10-K, 8-K, Form 4, 13F, etc.), returns
the data object's context with extracted financial data and section information.

Accepts:
- Accession number: "0000320193-23-000077"
- SEC filing URL: "https://www.sec.gov/Archives/edgar/data/320193/000032019323000077/..."
- Any URL or text containing an accession number

Examples:
- Filing by accession: input="0000320193-23-000077"
- Filing from URL: input="https://www.sec.gov/cgi-bin/browse-edgar?accession=0000320193-23-000077"
- With full detail: input="0000320193-23-000077", detail="full\"""",
    params={
        "input": {
            "type": "string",
...
async def edgar_context(
    ...
    try:
        ...
        next_steps = []
        if source == "filing":
            next_steps.append("Use edgar_filing with sections to read specific content")
        if obj_type:
