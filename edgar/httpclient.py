...
def _is_loopback_discard_proxy(value: str) -> bool:
    """Detect the common loopback discard proxy (127.0.0.1:9 / localhost:9)."""
    try:
...
def _scrub_blocking_proxy_env() -> None:
    """Remove loopback discard proxy env vars that break outbound connections."""
    removed = []
    ...
    if removed:
        logger.warning(
            "Cleared loopback proxy env vars %s (detected 127.0.0.1:9).",
            ", ".join(removed),
        )
