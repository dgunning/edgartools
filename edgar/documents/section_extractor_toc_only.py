"""
Simplified TOC-only section extraction for SEC filings.

This version uses only Table of Contents analysis for section mapping,
eliminating the complexity of semantic anchor detection while maintaining
100% compatibility across all SEC filings.
"""
import re
...
class TOCOnlySectionExtractor:
    """
    Extract specific sections from SEC filings using only TOC analysis.
    
    This simplified approach uses Table of Contents structure to identify
    section boundaries, providing consistent behavior across all SEC filings
    regardless of whether they use semantic anchors or generated IDs.
    """
    ...
    def _analyze_sections(self) -> None:
        """
        Analyze the document using TOC structure only.
        
        This creates a map of section names to their anchor positions
        using only Table of Contents analysis.
        """
    ...
    def _is_sibling_section(self, element_id: str, current_section: str) -> bool:
        ...
        if 'item' in current_section.lower() and 'item' in element_id.lower():
            ...
            other_item = re.search(r'item[_\s]*(\d+)', element_id, re.IGNORECASE)
