...
- **`get_total_liabilities()` returned liabilities *and equity*, so `debt_to_assets` came back as exactly 1.0.** The getter matched labels with `r'Liabilities$'` and took the first hit, which is "TOTAL LIABILITIES AND SHAREHOLDERS' EQUITY" on a balance sheet that reports no standalone `us-gaap:Liabilities` total — so NIKE's FY2026 10-K reported its $38.410B of total assets as its liabilities, against filed components of $23.545B and an actual ratio of 0.613. The bug was already in the fixture corpus and unasserted: Coca-Cola across three fixtures and Amazon all returned a liabilities figure exactly equal to their total assets. The getter now identifies the concept, and a filing that reports no total returns `None` rather than a substitute — for Coca-Cola and Amazon `us-gaap:Liabilities` is tagged only per segment, so no consolidated total exists to return. Deriving one needs a noncontrolling-interest rule to be right in general and is tracked as bead edgartools-y4yp.5. (GH #1279, bead edgartools-y4yp.1)
- **`get_revenue()` returned a component of revenue instead of the filed total.** It asked the standardization mappings for `Contract Revenue` before `Revenue`, but a contract-revenue fact can be one part of `us-gaap:Revenues` rather than the top line: Cato files both in the same face statement, with a calculation linkbase defining Revenues = RevenueFromContractWithCustomerIncludingAssessedTax + IncomeOther, so all three years were understated by exactly the other-income component ($7.7M in FY2023). `Statement.REVENUE_CONCEPTS` already ordered these total-first, so the same object's `get_revenue()` and `analyze_trends()` disagreed; they now agree. Filers whose only top line is contract revenue are unaffected — measured across the corpus, no filing resolves both concepts to different values. (GH #1294, bead edgartools-y4yp.2)
- **`get_shares_outstanding_basic()` returned `None` for shares present in the statement.** It routed through the one row-lookup helper of three that neither dropped abstract rows nor looked past its first match. An abstract heading carries no value and sorts above the row it introduces, so Auburn National's `WeightedAverageNumberOfSharesOutstandingBasicAbstract` shadowed the populated row and the helper moved on to the next *pattern* rather than the next matching *row* — 3,498,030 and 3,510,869 were both reported as missing. The helper now excludes abstract rows and iterates its matches, as its two siblings already did. (GH #1291, bead edgartools-y4yp.3)
- **A filed context whose ID ends in `_1` collided with an internal duplicate-occurrence index, and the parser silently dropped one of the two facts.** Fact keys were built as `element_id_context_ref[_instance_id]`, joined with `_` — which is legal inside an NCName, so `(concept, 'D20250630_1')` and duplicate 1 of `(concept, 'D20250630')` produced the same key and `facts_dict[key] = fact` overwrote. Naming the quarterly context `<ytd>_1` is a common filer-agent convention: ClearOne's Q2 2025 10-Q parsed 584 of its 590 filed facts, and the six lost were the quarter's own revenue ($1,916,000), net income ($-4,572,000) and inventory write-downs, leaving only the six-month figures — so a query for Q2 revenue returned no row at all. The parts are now joined with `|`, which neither a QName nor an NCName admits. (GH #1295, bead edgartools-jj77.1)
- **A concept whose name merely ended in `unit` was discarded as a structural element.** `_extract_facts` skipped any element whose tag ended with `context`/`unit`/`schemaRef`/…, testing a bare suffix against the expanded name, so `ctso:NumberOfSharesInAunit` was dropped before its `contextRef` was read — the sole occurrence of that fact in each of two Cytosorbents filings. Two copies of that set existed and only one was right: the counting pass compared the half-qualified `'}unit'`. Both are now one fully qualified set compared by equality, since a suffix cannot distinguish a filer's concept from a structural element. (GH #1293, bead edgartools-jj77.2)
- **Fact DataFrames deleted a fact denominated in a different currency when its number happened to match.** `_deduplicate_facts` classified duplicates on concept, context, value and decimals with no unit, which XBRL 2.1 §4.10 requires. Aebi Schmidt reports a CHF 10,000,000 and a EUR 10,000,000 shareholder loan in one context; `execute()` returned all four facts across the two instants and `to_dataframe()` returned two, dropping both EUR rows without a warning. The unit now participates in the key, so the repeated same-unit tags that motivated the policy still collapse: BorgWarner's four physical tags carrying two currency observations come back as two rows. No frame in the 33-filing fixture corpus changes size. (GH #1282, bead edgartools-jj77.3)
- **Stitched fact DataFrames deleted whichever columns the matched rows left empty, so narrowing a query changed the shape of the table.** `StitchedFactQuery.to_dataframe()` is the third implementation of a construction whose two siblings had already had this removed, and it still called `dropna(axis=1, how='all')`. Measured across five stitched pairs (Apple, Coca-Cola, Microsoft), it deleted a column on every one: `by_statement_type('IncomeStatement')` lost `period_instant` 5 times out of 5, because income-statement rows are all durations, and `by_statement_type('BalanceSheet')` lost `period_start` 5 times out of 5, because balance-sheet rows are all instants — so `df['period_start']` raised `KeyError` on a balance sheet purely because of which rows matched. An empty result returned a bare frame with no columns at all. The column set and dtypes now follow the query's configuration, per `engineering/decisions/facts-dataframe-schema.md`, and a column no row populated comes back null rather than vanishing, using a nullable dtype so a missing flag cannot read as `False`. Unlike the two sibling paths this one needs no breaking half: nothing here flips dtype across queries. `source_filing_index` is declared nullable `Int64`, which is what it is — and which nothing can be relying on, since it was deleted from every result. (bead edgartools-trhe)
- **`pivot_by_period()` on company facts discarded facts that collided on a key too narrow to tell them apart, and for an IFRS filer discarded nearly all of them.** Two independent keys compounded. `_deduplicate_facts` grouped on concept and period with no unit, so `ifrs-full:AverageForeignExchangeRate` — which LPA files once per currency — became a single group from which one fact survived; 19 distinct facts were lost on that 20-F, and the survivor no longer identified its currency, so a Colombian peso rate read as an unqualified number. The pivot then indexed on the label alone, and SEC company facts carry no label at all for IFRS concepts (all 177 of this filer's `ifrs-full` tags have `label: null` upstream), so 765 of 768 facts shared the label `''` and the entire filing pivoted into a two-row table. The unit is now part of a fact's identity for deduplication, and the pivot widens its row key — by the concept where the label cannot identify the row, then by the unit where one concept is filed per currency — applying each discriminator only to the rows that actually collide, so a row that never collided keeps its plain name. Any collision that remains is logged rather than silently resolved. The filing now pivots to 188 rows with all three currencies present at their filed values. The 21 facts still sharing a cell are a different key — `_format_period_label` maps monthly durations and the full year onto one "FY 2024" — tracked as bead edgartools-51xd. (GH #1196, bead edgartools-6byc)
- **Stitched fact rows named no source filing, and every row reported the newest filing's fiscal focus.** `_determine_source_filing` was a stub that returned `None` unconditionally, so `source_filing_index` was never populated and `by_filing_index()` — both declared public — matched nothing for any argument; a filter that silently returns no rows reads as "no matching data". Separately, the fiscal metadata was read from `xbrl_list[0]` for every period, so on an ORCL 10-Q stitched with a 10-K filed two years later, the quarter ended 2024-08-31 was reported as `fiscal_year=2026, fiscal_period=FY` — a focus taken from a filing that does not cover it. Rows now carry the filing each period actually came from, taken from the stitcher, which already decides it when selecting periods rather than being recomputed alongside it: the two answers differ, because a 10-K lists three years of comparatives among its reporting periods, and recomputing attributed all 183 rows of a two-filing Apple stitch to the newest filing where the stitcher knows 92 of those columns came from the older one. On the reported pair, 215 rows now split 115/100 across the two filings and the quarter reports its own 10-Q's Q1 FY2025. Fiscal metadata is still the source filing's declared focus rather than an identity derived from the period's own duration, which is tracked separately as bead edgartools-51xd. (GH #1205, bead edgartools-qbm7)
- **The entity fact DataFrames changed shape and dtype according to which rows a query matched.** `FactQuery.to_dataframe()` and `EntityFacts.to_dataframe()` both built their frame with `pd.DataFrame(records)`, which infers the column set and every dtype from the rows that came back, so a consumer's filter could break by filtering differently — no upgrade involved. Narrowing to a concept whose values are whole numbers flipped `value` from `float64` to `int64`, changing division semantics and the null sentinel; measured across 12 companies in different industries (344,769 rows), it flipped on 16 of 36 narrowing queries. A query matching nothing was worse: it returned a bare frame with zero columns, so `df['value']` raised `KeyError` rather than yielding an empty typed column, which a real query reaches — Main Street Capital reports no `Revenues` concept. Both methods now declare their columns and order, extending the rule already applied to the XBRL path in `engineering/decisions/facts-dataframe-schema.md`: the shape is a function of the query's configuration, never of the matched rows. `value` is pinned to `float64`, which moves nothing a caller depends on — the unfiltered surface is already `float64` on all 12 companies — and a column no returned row populated is materialized as null rather than vanishing, using a nullable dtype so a missing `is_audited` cannot come back as `False`. `fiscal_year` keeps the `int64` it reports today; making it nullable `Int64` moves the default's sentinel and is held for 6.0 (bead edgartools-7wtj.1). (bead edgartools-7wtj)

- **Line-item tags resolved to `Total*` concepts, so a standardized statement returned several rows all claiming to be the same total.** `us-gaap:BankOwnedLifeInsurance` resolved to "Total Assets" at confidence 0.319; on Citizens Financial's balance sheet three rows claimed to be total assets and the first gave $7.9B against the filed $226.4B. The guard is `is_total`, not a confidence threshold: confidence is not bimodal across the index (1,504 entries sit at exactly 0.5), and within the entries that claim a total it does not separate right from wrong either — `MembersEquity` and `PartnersCapital`, the total equity of an LLC and of a partnership, sit at 0.5 beside the bogus entries, so a threshold would strip the totals of exactly the non-corporate filers. A tag whose entry says it is not a total no longer resolves to one; 74 tag spellings stopped claiming a total they are not, and no mapping now resolves to anything different. Separately, an IFRS tag with no entry of its own was falling back to the US-GAAP index by bare name — that is how `ifrs-full:IntangibleAssetsAndGoodwill` became "Total Non-Current Assets" — and IFRS tags now answer only from IFRS entries, with both prefix spellings resolving. Entries the generator marked `is_total` that are themselves fuzzy matches remain wrong and need fixing upstream in edgar-storage. (GH #914, bead edgartools-gnx5)
- **`render(standard=True)` returned a DataFrame with no standardization in it.** Standardization deliberately preserves the filer's label and reports the standard concept beside it, and the renderer wrote that concept into each row — but `RenderedStatement.to_dataframe()` never emitted it, so the documented way to ask for standardization produced a frame indistinguishable from `standard=False`. The column now follows the flag rather than the data: present for every standardized statement, absent otherwise, so a caller's column set cannot shift with the rows that came back. Filer labels are unchanged, which is the intended behaviour and is now pinned by a test. (GH #1083, bead edgartools-t3zh)
- **A statement row could report a different concept's number, and the row that owned it was deleted.** `Statement.get_raw_data()` applies `_merge_complementary_rows()` on top of `get_statement()`. That function exists to join a line item a company renamed between years, and it matches rows on their LABEL, checking level and dimension status but never concept identity. For a dimensional row the label is the *member's* label, which every concept broken down by that member shares, so it identifies nothing; and complementarity was no guard either, since an instant fact and a duration fact can never collide. In Apple's FY2022 income-taxes note the count of Irish subsidiaries under EU State Aid absorbed the $13.1B assessment and the $12.7B still outstanding, and the loss-contingency row holding those figures was dropped from the statement. Measured across the 2,052 statements in the fixture corpus: 215 merges, of which 213 combined two different concepts — every one a dimensional row, one of them merging a dollar amount with a term in years — while not a single non-dimensional row merged across concepts anywhere. A dimensional row must now also match on concept, and no row merges across period types; 2 merges remain, both of one concept with itself. (GH #1206, bead edgartools-j7iz)
- **Every per-share unit was parsed as plain currency, discarding its denominator.** `_extract_units` searched for a measure with a descendant lookup before testing for `xbrli:divide`, and a well-formed divided unit necessarily contains measures inside its numerator and denominator — so the search always matched the numerator and returned, making the divide branch unreachable on any real filing. `usdPerShare` was recorded as `{'type': 'simple', 'measure': 'iso4217:USD'}`, leaving a per-share amount indistinguishable from a dollar amount, and the corpus contained zero units of type `divide`. Fixing the parser alone would have silently broken currency resolution, because a divided unit has no `measure` key and two consumers tested for one: verified without the consumer fix, `get_currency_for_fact()` returned `None` for basic and diluted EPS on both Apple and JPMorgan, so every per-share cell would have lost its currency symbol. The unit-to-currency rule now lives once, in `edgar.xbrl.core.unit_currency`, rather than in each of the three places that needed it. (GH #1170, bead edgartools-uetp)
- **A presentation role's second root, and every fact beneath it, was missing from the statement.** The parser finds all of a role's roots and builds every subtree, but `PresentationTree` carried a single `root_element_id` and `get_statement()` walked from that one — so the rest sat in `all_nodes` unreachable, and since the root list is sorted for determinism, which subtree survived was alphabetical rather than the filer's intent. Union Pacific's FY2012 leases note returned 13 rows from a 22-node tree with its entire operating-lease schedule absent, and the same construction truncated a primary face statement: net income was missing from that filer's consolidated statement of comprehensive income, so it did not reconcile. Across the committed fixture corpus 9 of 2,052 roles are multi-root; recovering them adds 106 rows and changes nothing in the other 2,043. `PresentationTree` now carries `root_element_ids`, with `root_element_id` kept as the first. (GH #1245, bead edgartools-0q0d)
- **A concept presented twice in one role reported the last occurrence's parent and indentation on both rows.** Presentation nodes are stored keyed on concept, so each repeated occurrence overwrote the previous one's parent and depth, and the row builder read them back off that shared node. JPMorgan's FY2013 earnings-per-share note presents net income available to common stockholders under both the basic and the diluted two-class-method sections; both rows named the diluted parent. Values and labels were always correct, which is why this went unnoticed — a consumer reading numbers was fine, one reconstructing the tree was not. `all_nodes` keeps its concept keying, since every other caller uses it as a lookup or membership set; the row now takes its position from the walk, which already threads the path down the recursion. Corrects 77 rows' parent across 41 roles and 71 rows' level across 12. (GH #1231, bead edgartools-f07v)
- **A negated label rendered with the filed positive sign, and on the Facts API nothing was ever negated at all.** Two independent matchers decided whether a `preferredLabel` role means "negate for display". The statement path required the role to start with `negated` or contain `/role/negated`, which the legacy xbrl.us role `http://xbrl.us/us-gaap/role/label/negated` satisfies neither of — its extra `/label` segment defeats the substring test — so every negated line in a filing using those roles, common in 2009-2011, kept the filed positive sign: Apple's FY2010 10-K showed purchases of marketable securities as +57,793M where the filed document shows (57,793). The Facts API carried a second matcher, an exact-match whitelist of eight strings covering only the 2003 namespace, and filings use the 2009 one, so it matched nothing on any filing — `preferred_sign == -1` appeared on 0 of 1,075 Apple FY2023 facts, 0 of 1,059 Tesla and 0 of 1,526 Union Pacific, and now on 130, 84 and 202. Both paths now read one matcher, `is_negated_label_role`, which tests the role's local name and so covers every namespace version, the legacy LRR roles, and the modern `negatedTotalLabel` / `negatedNetLabel` the whitelist omitted. Which rows a current filing negates on the statement path is unchanged. (GH #1249, bead edgartools-hxnw)
- **`Statements.to_dataframe()` and `Statement.to_dataframe()` returned different signs for the same line.** The singular accessor applies the presentation sign; the plural one goes through `render().to_dataframe()`, which reads the cell's value, and the sign was applied only inside the cell formatter that produces display strings. Apple's FY2023 capital expenditures came back +10,959,000,000 from one accessor and -10,959,000,000 from the other, with nothing to say which to believe. The sign now lives in one function called by both, so 13 rows of that cash flow statement change and the two accessors agree on every cell. `RenderedStatement.to_dataframe(presentation=False)` returns the raw filed values for anyone who relied on the old behaviour, and `Statements.to_dataframe()` takes the same flag. (GH #1248, bead edgartools-5ztr)
...
- **Period and statement selection silently ignored what the caller asked for, in three places.** `select_periods(max_periods=N)` never re-applied the cap: the selectors deliberately over-fetch to `N * 3` (issue #464) so data-quality filtering has candidates to choose between, but that filter only *removes* periods below the threshold and never truncates, so a healthy filing where every candidate qualifies got the whole over-fetch back. Microsoft's FY2024 balance sheet returned 10 periods for `max_periods=4`, and the count flows straight through `XBRL.render_statement()` into `RenderedStatement.periods` and `Statement.to_dataframe()`. The over-fetch is correct and stays; only the missing cap was added. Named balance-sheet period views admitted incidental instants — contexts a filing declares but reports no balance-sheet facts against — because the instant branch of `get_period_views()` had no fact-sufficiency check while the duration branch three lines below did: Microsoft's "Current vs. Previous Period" rendered 2024-06-30 beside an empty 2023-12-31, with the populated 2023-06-30 comparative excluded, and Coca-Cola FY2023 showed the same shape. The instant branch now reuses the same statement-aware filter the default path already applied, which is what made the two paths disagree in the first place; a view that can only be filled with incidental instants is no longer offered. And `parenthetical=True` was discarded during resolution: the standard-name matcher sits first in the cascade at 0.95 confidence against a 0.9 threshold and never inspected the flag, while two later matchers score on concepts and role text alone and answered at 0.85 against a 0.6 threshold, so an explicit request returned the ordinary statement with no indication the flag had been dropped. Apple, Microsoft and Coca-Cola all declare a parenthetical balance sheet and all three returned the face statement. The `-80` parenthetical penalty added for `edgartools-8ad8` is inverted for an explicit request rather than removed, so parenthetical roles are still not chosen by default, and a matcher that cannot honour the flag no longer answers a request that carries it. A filing that declares no parenthetical role for the requested statement now raises `StatementNotFoundError` instead of substituting the face statement. The `BalanceSheet` and `CashFlowStatement` role patterns also now allow the plural "Statements of…" that `IncomeStatement` already allowed, without which Union Pacific's parenthetical balance sheet — typed `None`, with a share-count primary concept, so identifiable only by its role name — matched no pattern at all. (GH #1221, #1222, #1246, beads edgartools-rj4b, edgartools-zt8u, edgartools-z0y2)
...
- **A 273-day year-to-date figure was appearing in a quarterly column.** `cash_flow_statement(period='quarterly')` reported Snowflake's `PaymentsToAcquireInvestments` for Q3 FY2022 as `3,042,396,000`, the nine-month cumulative total, against a discrete quarter of `1,053,763,000` — a 2.89x overstatement. Two causes: comparative re-filings were discarded as inconsistent, which removed the six-month operand the quarter is derived from, and the quarterly path had no duration filter to mirror the annual one. (GH #1180)
- **`TTMCalculator.quarterize()` returned periods that were not quarters.** Deriving a quarter pairs a cumulative fact with an earlier one, and when the two came from different fiscal cycles the subtraction produced a real number over a nonsense interval: Snowflake's goodwill yielded `-45,711,000` across 457 days labelled Q2 FY2025. Everything `quarterize()` returns is now checked to span a quarter. (GH #1197)
- **`latest_periods(1)` returned three years of facts.** Periods were keyed on `(fiscal_year, fiscal_period)`, which is the SEC's filing focus rather than the period itself, so the 2022, 2023 and 2024 comparatives in Logistic Properties' 20-F — all tagged `fy=2024, fp=FY` — counted as one period. Periods are now keyed on the dates the facts cover, and annual-ness comes from the duration. (GH #1185)
- **A stitched cash flow presented an unchanged annual value as Q4.** With `discrete_quarters=True`, a concept present on the 12-month period but not the nine-month one had nothing to subtract and kept its cumulative value, while the period was relabelled a quarter regardless. Meta's FY2024 "Deferred income taxes" changes concept between the 10-K and the 10-Q, so `$(4.738)B` was shown as Q4. A quarter that cannot be derived is now dropped, so the cell reads empty rather than wrong. (GH #1179)
- **Form D sales-compensation recipients lost their ZIP codes.** `recipient.address.zipcode` was always `None`: the parser looked for a child element named `30361` — the sample ZIP from the code's own docstring, pasted in where the tag name belongs. The four recipients in the 1685 38th REIT notice now read `30361` as filed. Related-person addresses were never affected. (GH #1193)
- **`FormD.is_new` was inverted, so every base notice rendered as `FORMD/A` and every amendment as `FORMD`.** The field was assigned the filing's `isAmendment` answer, which asks the opposite question — AP Fund IV, a base `D`, reported `is_new=False`. `to_context()` now takes its heading from `submission_type`, so the heading cannot contradict the field beside it. (GH #1192)
- **`Form144.nothing_to_report` returned a truthy `"N"`.** It carried the raw SEC flag against a `bool` annotation, and `bool("N")` is `True`, so `if form.nothing_to_report:` read a notice reporting a sale as reporting none — accession `0001958244-23-000454` has one prior-sale row. `Y`/`N` now parse to `True`/`False`, and an unanswered flag is `None` rather than a default. Callers comparing against the string need updating. (GH #1195)
...
- **`get_financials()` no longer goes quiet on a filing with no XBRL.** It returned a truthy `Financials` whose `income_statement()`, `balance_sheet()` and `cash_flow_statement()` all answered `None` — `Company(104599)`, Circuit City's 2008 10-K, is the case. It now emits a `FutureWarning` and will raise `XBRLFilingWithNoXbrlData` in 6.0; set `EDGARTOOLS_STRICT_ERRORS=1` for that behaviour today. `filing.xbrl()` still returns `None` quietly, which is a true absence rather than a failure.

### Fixed

- **`get_revenue()` returned a years-stale value after a company migrated its XBRL tag.** The first variant with any matching fact won, regardless of period, so NVIDIA answered FY2022 (`26,914,000,000`) instead of FY2026 (`215,938,000,000`). Candidates are now gathered across all variants and ranked by recency. Every standardized getter built on the same helper was exposed. (GH #1149)
- **`get_revenue()` returned the ASC-606 slice for insurers and banks.** MetLife tags both the contract-revenue slice (`2,436,000,000`) and consolidated `Revenues` (`77,084,000,000`) for FY2025, and the slice ranked first — a 32x understatement, with an income statement showing less revenue than operating income. A same-period candidate that dwarfs the ranked pick is now taken as the consolidated total. Revenue only: net income's variants are not slices of one another. The statement builder held a second copy of the priority list and now shares one threshold with the getter.
- **`edgar_trends` returned whatever concept contained the word.** Concepts were matched as substrings, so `"Revenue"` also matched `CostOfRevenue` and `DeferredRevenue`; rows were truncated before the period-type filter, so the correct annual fact could be cut from the window; and quarterly facts labelled `FY` entered annual series under the same year label as the real annual figure. Apple's revenue read as falling from $383B to $7.7B. Concepts are now matched exactly, filtered before truncation, and period type comes from the reporting window rather than `fiscal_period`. Revenue trends take the same consolidated-total cross-check as `get_revenue()`. (GH #1138)
- **A fund series asked for its filings answered with the whole trust's.** `FundSeries.get_filings()` delegated to the fund company, and a trust files one report per series under one CIK, so every series returned the same list and `filings[0]` belonged to whichever sibling filed last: Vanguard's Extended Market Index fund, which excludes the S&P 500 by construction, was handed the 500 Index fund's portfolio. It now resolves its own series through SEC browse-edgar and returns empty rather than falling back to the trust. `Fund.get_portfolio()` and `get_latest_report()` took the same trust-wide path and are fixed with it. `Fund.get_filings()` without `series_only` still returns the whole trust. (GH #1143)
- **`year` and `quarter` accept a list on the fund-series path.** A list was untranslatable, silently dropped, and the caller received the series' entire unfiltered history. An unusable pair now raises rather than answering with everything. (GH #1143)
...
- **`find_bdc("Ares")` — the example in its own docstring — returned nothing.** Names were scored with whole-string `fuzz.ratio` against a threshold of 50, so a short query against a longer registered name missed: `ARES` against `ares core infrastructure` scores 28.6. Candidates already come from an exact word index, so they are now scored with `token_set_ratio`. (GH #1145)
- **`edgar_fund(action="portfolio")`'s `total_value` summed share counts, not dollars.** None of the candidate columns existed on `FundReport.investment_data()` — the dollar column is `value_usd` — so it fell through to `balance` and summed raw units. VFINX reported a `total_value` of $14.4B against a top-5-holdings sum of ~$366B. Per-holding records were unaffected. (GH #1148)
