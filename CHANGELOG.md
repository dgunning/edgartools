...
- **`get_revenue()` reported the ASC-606 slice for insurers and banks, not consolidated revenue.** Ranking candidates by recency settles which *year* to answer from; among concepts tagged for that same year the variant list decides, and that list orders *names*, not what the names measure. MetLife tags both `RevenueFromContractWithCustomerExcludingAssessedTax` (`2,436,000,000`) and `Revenues` (`77,084,000,000`) for FY2025, and the contract tag is ranked first — so revenue came back 32x understated, and the income statement showed $2.4B of revenue above $6.1B of operating income, which cannot be true and also disagreed with `get_revenue()` for the same company and year.

  A same-period candidate that dwarfs the ranked pick is now treated as the total the pick is a slice of. Magnitude is only ever a cross-check on the ranked pick, never the ranking itself: taking the largest outright would prefer `IncludingAssessedTax` over `Excluding`, and gross over net. It is opt-in per concept family and off everywhere else — net income's variants are not slices of one another, and preferring the larger `ProfitLoss` over `NetIncomeLoss` would quietly change whose earnings are reported. General Mills is unchanged at `18,424,600,000`: its correct value is the lower-ranked concept in the newer period, so neither "newest wins" nor "largest wins" is safe alone.

  The statement builder holds a second copy of the same priority list and had drifted from the getter. Both now share one threshold, and the revenue row is named after the concept its value came from rather than the highest-priority concept merely present in the filing.

- **`edgar_trends(concepts=["revenue"])` did not return revenue.** What it returned depended on `periods`, was wrong for some companies at every value, and never signalled anything was off. Two defects composed. `EntityFacts.time_series` passes `exact=":" in concept` down to `FactQuery.by_concept`, and no value in the tool's `CONCEPT_MAP` contained a colon — so every lookup took the substring branch and matched every concept *containing* the name: `"Revenue"` pulled in `CostOfRevenue`, `DeferredRevenue` and 12 others for NVIDIA, while `"Assets"` and `"Liabilities"` matched their Current, Other and Intangible variants. Then `time_series` sorted by filing date and truncated to `periods * 3` rows *before* the tool filtered to `fiscal_period == "FY"`, so the correct annual fact could be cut from the window entirely and the value-descending dedup picked the largest surviving wrong concept. Apple read as revenue falling from $383B to $7.7B; NVIDIA's FY2026 came back as cost of revenue.

  Revenue trends also take the same consolidated-total cross-check as `get_revenue()`, sharing one threshold rather than a second copy of the rule. Before this, `edgar_trends` reported MetLife's FY2025 revenue as `2,436,000,000` while `get_revenue()` reported `77,084,000,000` — two surfaces disagreeing about the same company and year.
- **`Fund.get_portfolio()` and `get_latest_report()` still answered with a sibling fund's report.** Follow-up to the `FundSeries.get_filings()` fix: both chain through `get_filings()` without `series_only`, so they took the trust-wide path and `Fund("VEXMX").get_latest_report()` returned `S000002846`'s report — the same harm through the documented holdings entry point, which also backs MCP `edgar_fund(action="portfolio")`. They now request their own series whenever the `Fund` names one; a `Fund` built from a multi-series trust CIK names no series and is unchanged.

  Also on the series path, `year` and `quarter` now accept a list, as they do on the entity path. A list was untranslatable in `_series_filter_kwargs`, silently dropped, and the caller received the series' entire unfiltered history — `series.get_filings(year=[2023, 2024])` returned all 485 filings, exactly what no filter at all returns. A pair that still cannot be expressed as date ranges now raises `ValueError` rather than answering with everything. (GH #1143)

- **`edgar_trends(concepts=["revenue"])` did not return revenue.** What it returned depended on `periods`, was wrong for some companies at every value, and never signalled anything was off. Two defects composed. `EntityFacts.time_series` passes `exact=":" in concept` down to `FactQuery.by_concept`, and no value in the tool's `CONCEPT_MAP` contained a colon — so every lookup took the substring branch and matched every concept *containing* the name: `"Revenue"` pulled in `CostOfRevenue`, `DeferredRevenue` and 12 others for NVIDIA, while `"Assets"` and `"Liabilities"` matched their Current, Other and Intangible variants. Then `time_series` sorted by filing date and truncated to `periods * 3` rows *before* the tool filtered to `fiscal_period == "FY"`, so the correct annual fact could be cut from the window entirely and the value-descending dedup picked the largest surviving wrong concept. Apple read as revenue falling from $383B to $7.7B; NVIDIA's FY2026 came back as cost of revenue.

  A third defect surfaced while verifying the fix: `fiscal_period` does not separate annual facts from quarterly ones. Companyfacts labels quarterly facts `FY` as well — General Mills' 90-day Q3 and its 370-day fiscal year are both `fiscal_period == "FY"` with `fiscal_year == 2026` — so a quarter entered the annual series and rendered under the same year label as the real annual figure, giving two rows both reading "2026", one of them $4.4B against the actual $18.4B. The period type is now decided by the reporting window (`duration_days`), which `time_series` returns for exactly this purpose; instants carry no duration and belong in either series, so they are kept.

  The answer therefore tracked filing cadence rather than anything the caller asked for — NVIDIA's `periods=8` result was correct when the issue was filed and became wrong after a 10-Q landed. Concepts are now fully qualified, so matching is exact; the period-type filter runs before the row limit; and each entry lists its variants in priority order, so a company that migrates tags mid-series no longer leaves a hole. All eight mapped concepts were affected. (GH #1138)
...
- **A BDC search for the one word anyone knows the fund by found nothing.** `find_bdc("Ares")` — the example in `find_bdc`'s own docstring — returned an empty result set, as did `find_bdc("Antares")` and `find_bdc("Owl")`, while `find_bdc("Blackstone")` returned two. Names were scored with `fuzz.ratio`, a whole-string similarity, against a threshold of 50, and a BDC's registered name is longer than the part of it anyone types: `ARES` against `ares core infrastructure` scores 28.6 and `ANTARES` against `antares private credit` scores 48.3, missing by 1.7, where the ten characters of `BLACKSTONE` reach 57.1. Whether a search worked came down to the length of the query relative to the name. The candidate set is already built from an exact word index, so every name that reached scoring had matched a whole word — those hits are now scored with `token_set_ratio`, which recognises them. Not `partial_ratio`, which scores `ARES` against `ANTARES PRIVATE CREDIT` at 100 because one is a substring of the other; token matching gives that pair 30.8 and keeps it out. Exact word hits all score 100, so a sub-point tiebreak toward the closer whole name keeps `ARES CAPITAL CORP` ahead of `ARES CORE INFRASTRUCTURE FUND` for the query `Ares`. Ticker matching is untouched. (GH #1145)
- **`edgar_fund(action="portfolio")`'s `total_value` summed share counts, not dollars.** The candidate column list for the aggregate total was `['value', 'market_value', 'val', 'balance']`; none of the first three exist on `FundReport.investment_data()` (the real dollar column is `value_usd`), so it fell through to `'balance'` and summed raw share/unit counts. For VFINX (Vanguard 500) this produced a `total_value` of $14.4B against a top-5-holdings sum of ~$366B — a "total" smaller than a subset of its own holdings. Per-holding records were unaffected; only the aggregate was wrong. (GH #1148)
...
- **`get_revenue()` (and other standardized concept getters) could return a years-stale value when a company migrated its XBRL tag over time.** `_get_standardized_concept_value` tries each concept variant in a fixed priority order and returns as soon as ANY variant has ANY matching fact, never comparing across variants for recency. NVIDIA tagged revenue as `RevenueFromContractWithCustomerExcludingAssessedTax` through FY2022, then switched to `Revenues`; since the old tag is tried first and still has real (if 5-year-stale) data, `get_revenue()` returned NVDA's FY2022 figure ($26,914,000,000) instead of its current FY2026 figure ($215,938,000,000). Candidates are now gathered across every variant and ranked by recency, falling back to an older candidate only if unit normalization fails for it. (GH #1149)
