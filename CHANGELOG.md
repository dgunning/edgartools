...
- **`Document.to_dataframe()` raised on every real annual and quarterly report, from inside pandas and numpy.** It failed on 10 of the 11 filings in the 6.0 performance corpus, with three different messages depending on which pair of frames pandas happened to align first — `cannot join with no overlapping index names`, `Cannot cast array data from dtype('float64') to dtype('int64') according to the rule 'safe'`, and `Index data must be 1-dimensional`. The only entry that worked was an ABS-15G, which is a single flat table: the degenerate case.
  ...
  One cause underneath. A filing's tables do not share a schema — Meta's FY2024 10-K has 71 tables whose column indexes are 1, 2, 3, 4, 10 and 17 levels deep — and pandas cannot align a flat `Index` with a `MultiIndex`, or two `MultiIndex`es of different depths. The document-level export now flattens each table to single-level string column names before stacking, so all 11 corpus filings convert, from Meta's 565 rows to Fannie Mae's 46,714. Row labels that a table had moved into its index come back as a column instead of being discarded by the concatenation, and a label whose name collides with a real column is kept rather than refused — Citigroup and Morgan Stanley both caption a table with the same text they use as a column header.

  `Document.to_dataframe()` is a document-level view and is deliberately lossier than `TableNode.to_dataframe()`, which still keeps a table's real header structure. Use `document.tables[i].to_dataframe()` when a single table's headers matter.

- **A table whose first header text repeats built its row index out of every matching column, giving `None`-padded tuples instead of labels.** `TableNode.to_dataframe()` moved the label column into the index with `set_index(df.columns[0])` — a lookup by *label*. Filings repeat a header across spacer columns routinely, and when they do the lookup matched all of them, so a row label came back as `('Gross written premiums by line of business:', None, None)` rather than the string. Where the match came back two-dimensional it raised `Index data must be 1-dimensional` outright, which is what broke Tesla's FY2023 10-K.

  The first column is now taken by position, which is what the surrounding code always intended. Across the 11-filing corpus this changes 711 of 1,940 tables and every one of them in the same way — index values go from tuples to scalars — plus one table that used to raise and now converts; nothing else moves.

  A label column is often laid out across two or three physical columns sharing one header, with only the first holding the label, so once it becomes the index the rest are empty columns named after it. Those are dropped, but only when they are genuinely empty: 77 corpus tables have a same-headed column carrying a percentage or a rate, and those stay as data. Only 31 of the 1,940 tables end up wider than before, against 444 if the empty ones were kept, and the corpus exposes exactly the same 1,026,693 non-blank cells either way — nothing is lost, it only stops being padding inside an index tuple.

  The same label lookup was also the source of the `PerformanceWarning: indexing past lexsort depth` messages that table work emitted dozens to hundreds of times per document (309 on Citigroup's 10-K, 294 on Regions'). There are now none.

- **A filing's beneficial-ownership table was read as its underwriting syndicate, so `lead_manager` came back as the column header `'Before Offering'` and the roster listed five individual directors.** The two tables have the same shape — a name column beside a `Number of Shares` column — so an ownership table satisfies the row-based-header test, and having no bank names in its rows it also satisfies the "trust the structure" branch, which accepts every row label as a firm.

  On Learn CW Investment Corp's S-1 (`0001140361-21-010426`) the roster was `'Before Offering'`, the sponsor `CWAM LC Sponsor LLC`, then `Robert Hutter`, `Adam Fisher`, `Greg Mauro`, `Alan Howard` and `Daniel H. Stern` — the directors named in the ownership table — with the one real underwriter, `Evercore Group L.L.C.`, last. Nothing was missing from the filing: its actual `Underwriter | Number of Units` table is there and names Evercore. An ownership table simply reached the extractor first.

  Fixed at the table rather than at the name, because no per-name check can do it. `is_plausible_underwriter_name('Before Offering')` is `True`, and so is every director's name — they are plausible firm names by any per-name test, which is why the wrong roster looked entirely reasonable. A table whose header region names beneficial ownership (`beneficial owner`, `beneficially owned`, `before offering`, `after offering`) is now skipped before any pattern is considered. Only the header region is read — real headers plus the first two rows, since these tables stack a second header row — and only label-length cells, so a genuine underwriter table carrying a footnote about beneficial ownership keeps its syndicate. Learn CW now returns `Evercore Group L.L.C.` and nothing else; across the 44-filing offerings eval corpus the `lead_bookrunner` facet is unchanged at 0.929 coverage and a zero bad rate.

- **An underwriter that files as a *division* of its broker-dealer was rejected as parser junk, so `lead_manager` returned `None` on every filing it led.** A boutique underwriter is often a division rather than a firm in its own right, and files under the full construction — `EF Hutton, division of Benchmark Investments, LLC`, `ThinkEquity, a division of Fordham Financial Management, Inc.`. The name guard treats a lowercase mid-name word as the signature of a leaking term fragment, and its allowlist of permitted lowercase tokens (`and`, `of`, `the`, `llc`, `inc`, …) had no entry for `division` or the article `a`. Both names are well under the guard's 80-character cap; they failed on that single token.

  A rejected name is dropped at both guard sites, so the roster came back empty and `RegistrationS1.underwriting.lead_manager` was `None` — no warning, no partial result. On Unicycive Therapeutics' S-1/A (`0001213900-21-035033`) the sole underwriter on the cover was the rejected name, so the filing reported no underwriters at all; it now returns `EF Hutton, division of Benchmark Investments, LLC`. Confirmed on four filings, and EF Hutton and ThinkEquity between them led a large share of small-cap IPOs in 2021-2023, so this was a population rather than an outlier.

  The guard arrived with the gh-868 fix, which stopped an Airbnb lock-up table header (`Earliest Date Available for Sale in the Public Market`) leaking into the roster. That defect stays guarded: the widened allowlist is pinned against six junk strings taken from real filings, all still rejected, because `for`, `in`, `to`, `issued` and `officers` remain disallowed. Found by sweeping 66 small S-1s.

- **A two-column table of contents read one column's page number as the other column's item number, inventing a section that truncated MD&A.** Both columns of a two-column TOC share one HTML row, so a row reads `['', 'Available Information', '10', '', '', 'Non-GAAP Financial Measures', '54']`. The scan for an item label walks every cell to the left of a link, so the right column's title reached back past the gap and took `10` — the *page number* of the left column's entry — for its item number.

  On Ambac's FY2022 10-K (`0000874501-23-000040`) that produced a `part_ii_item_10` section: a 10-K's Part II has no Item 10, and the anchor pointed at the "NON-GAAP FINANCIAL MEASURES" heading *inside* MD&A. Item 7 was cut off there, **158,411 → 149,459 characters**, and the remainder was filed under a key no caller will ask for. (Before section boundaries were resolved in document order the same phantom ran to 384,233 characters, through Items 7A, 8 and 9.)

  The scan now stops at the column boundary — but only on a TOC confirmed two-column, the same gate the Part-header scan uses. A conventional single-column row is `['Item', '1', 'Business', '5']`, where the label sits across the row's midpoint from the title, so refusing to cross it unconditionally would discard every item number. Ambac's `part_ii_item_10` is gone and its Item 7 is whole again; across the eleven-document benchmark corpus those are the only two entries that change, and the other 186 are byte-identical.
...
- **Sections ended at the next item *number* rather than the next section in the document, so filings that group their items out of numeric order both lost sections and silently mislabelled others.** A section ends where the next one physically begins, and that is not the same as the next item number: item numbering is what a filer calls its sections, document order is where it put them.

  Morgan Stanley's FY2024 10-K (`0000895421-25-000200`) groups Items 1B, 2, 3, 4 and 5 behind the financial statements, laying its items out as 1, 1A, 1C, 7, 7A, 8, 9, 9A, 9B, 9C, 1B, 2, 3, 4, 5, 10 … 15. Ending each section at the next item number failed in both directions at once. Item 1A ran from Risk Factors all the way to Item 1B's anchor near the back of the filing — 673,015 characters with MD&A, the financial statements and the controls items all filed under Risk Factors, at confidence 0.95 and with no warning — and Item 1C, a two-line cross-reference to the risk disclosures, was 600,239 characters for the same reason. In the other direction Item 1B's end resolved 75,978 elements *before* its own start, and an inverted pair yields nothing, so Items 1B and 5 were absent from `document.sections` entirely. Item 1A is now 73,167 characters ending where Risk Factors ends, Item 1C is the 160-character cross-reference, Item 1B is its single paragraph and Item 5 opens on "Market for Registrant's Common Equity".

  This is ordinary layout rather than a malformed filing. ODP's FY2025 10-K places Item 8 after the signature page, where the F-pages conventionally live, so `signatures` was absorbing 188,606 characters of financial statements while Item 8 produced nothing; Item 8 now holds the statements and `signatures` is 2,671 characters. Ambac's FY2022 10-K recovers Item 9B, which had been silent.

  The end boundary is now the nearest section anchor that *follows* the section in document order, derived from the already-resolved anchor elements at O(depth) each rather than by walking the document. Sections that share an anchor still skip past every equal anchor, so the Coeur Mining case behind GH #904 is unaffected, and boundary iteration order remains item order, so `document.sections` is still presented in the order a reader expects. Across the eleven-document benchmark corpus this changes eleven section entries in three filings and leaves the other eight documents byte-identical.

  One section shrinks for an unrelated reason. Ambac's two-column TOC flattens to `… Available Information 10 | Non-GAAP Financial Measures 54 …`, and the left column's page number is read as an item number, producing a phantom `part_ii_item_10` anchored inside MD&A. That phantom used to swallow 384,233 characters through Item 9; it is now bounded to 8,950, but because it sits inside Item 7 it also truncates MD&A from 158,411 to 149,459 characters. The phantom itself is a TOC-parsing defect tracked separately (edgartools-fhk1) — the text is still reachable, under the wrong key.
...
- **Wells Fargo's 10-K published its exhibit list as the financial statements, and Citigroup's items were unreachable by their canonical keys.** Two filings that every anchor-driven strategy failed on, for two unrelated structural reasons.
  ...
  Wells Fargo's FY2024 10-K (`0000072971-25-000094`) writes each item heading as a standalone one-row table — `ITEM 1A. | RISK FACTORS`. The parser classifies that lone row as the table's *header* row, so it lands in `TableNode.headers` and leaves `.rows` empty, and the pattern extractor's table strategy only scanned `.rows`. Finding no item headers anywhere, matching fell through to bare-title keywords: the `FINANCIAL STATEMENTS` heading inside Item 15's exhibit list claimed the `financial_statements` key with 42,462 characters of the wrong content. Wells Fargo incorporates its statements from the 2024 Annual Report, so Item 8 is a 261-character pointer, and ranking candidates by content size let the exhibit list win outright. Header rows are now scanned alongside body rows, and a header that names its item (`ITEM 8. FINANCIAL STATEMENTS`) is preferred over a bare title match regardless of span. The filing goes from one section of wrong content to all 23 items, each opening on its own heading; the exhibit list moves to `part_iv_item_15`. Seven items that had no vocabulary entry at all — 4, 5, 6, 9, 9B, 9C and 15 — were added to the 10-K schema, since on a filing whose only usable headers are `Item N` markers they were unrecoverable and their content was absorbed by the preceding item.

  Citigroup's FY2024 10-K (`0000831001-25-000029`) maps items to printed page ranges in a `FORM 10-K CROSS-REFERENCE INDEX` rather than labelling them in the body — the 16.7MB document contains two occurrences of the string "Item N" and no anchors. The index parse anchored on the heading and read to the first `</table>`, but Citi's index breaks across a page and resumes in a second `<table>`, so it stopped at 14 of 22 items, silently losing 9B, 9C and all of Part III. Two defects were hidden behind that early stop: `^Part\s+(I+|IV)` let `I+` claim the leading "I" of "Part IV" and label Item 15 as Part I, and `PageRange.parse` fed `"319-321*"` to `int()`, raised, and discarded Item 10's pages over a footnote marker. The parse now continues across adjacent index tables and recovers all 22 entries.

  That index is also now used to build sections. Previously it was parsed but never consulted by section detection, so `document.sections` held four non-canonical keys from keyword matching and a caller asking for `part_ii_item_7` got nothing while `mda` quietly held the MD&A. Citigroup now yields 11 canonically-keyed sections (`part_ii_item_7` opens on `MANAGEMENT'S DISCUSSION AND ANALYSIS` with 473,576 characters, against 234,483 under the old `mda` key), and `tenk.items` reports them. Text is extracted lazily, so parse time is unchanged. Items whose index row reads "Not Applicable" or carries only an incorporation-by-reference asterisk produce no section and `tenk['Item 15']` returns `None` — there is no such disclosure in the document, and an empty section would present absence as content.

  One known defect is left in place deliberately: Citigroup's own index cites pages 135-136 for Item 9A, but Controls and Procedures is on page 134 (135 is Forward-Looking Statements, 136 the auditor's report). The folio map is exact — 329 of 330 recovered, offset uniformly −1, zero outliers — so this is the filer mis-citing its own pages, not a mapping error. Snapping to the nearest matching heading is left to the low-confidence section-signalling work rather than added as a lone heuristic, and is pinned by a strict `xfail`.
...
- **10-K items fabricated from numbered index rows** — Freddie Mac's FY2025 10-K mapped every `part_*_item_N` onto an MD&A table caption (`obj['Item 11']` returned 30K chars starting at "Table 11 - Other Investments Portfolio") at full confidence: the bare row-number cells of the MD&A's "List of Tables" index (`11 | Other Investments Portfolio | 18`) fed the generic TOC scan as item numbers. Bare numbers are now ignored when the containing table's header row names the numbering ("Table | Description | Page") without an "Item" column — a genuine TOC heads its number column "Item" (Morgan Stanley) or carries no header at all, so headerless bare-number TOCs are unaffected. The same guard removes the phantom `part_i_item_5`/`part_i_item_6` on FMCC's 10-Qs that GH #905 could only flag-and-downgrade, and the surviving Part I items now anchor on the real MD&A and Market Risk content instead of table scraps. (GH #918)
