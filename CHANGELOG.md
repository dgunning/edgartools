...
- **`Company.get_facts()` re-downloaded companyfacts on every call, and the 30s `/submissions` TTL never took effect.** `_get_cache_rules()` filed every rule under `SEC_BASE_URL`, but httpxthrottlecache matches the request *host* against that key before the path, and `re.match(r'.*www\.sec\.gov', 'data.sec.gov')` is `None`. Both endpoints come from `SEC_DATA_URL`, so a fresh process logs `No patterns matched data.sec.gov` and goes to the network — including the 600s → 30s reduction made for GH #471. (GH #989)

- **A custom mirror lost caching on the host it configured, in four ways.** Cache keys are regexes matched against the request host, and they were built by hand: unanchored, so `.*mirror.example.org` also matched `data.mirror.example.org` and answered it with the base host's rules — the same bug as above, moved from sec.gov to mirrors — and hand-parsed, so `EDGAR_DATA_URL` given with a capitalised host, a port, or `user:pw@` credentials produced a key that no real request could ever match. Keys now come from `httpx.URL(...).host`, the same parser that produces the host at match time, and match it exactly. A mirror URL with no host at all is logged and cached under nothing, rather than borrowing another host's rules. Requires `httpxthrottlecache>=0.6.1`: caching `data.sec.gov` reaches an age check that raised uncaught when the local clock trails the origin's `Date` header, which 0.6.1 clamps (paultiq/httpxthrottlecache#43).
