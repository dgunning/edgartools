...
- **Two-column tables of contents put items under the wrong Part, overflowing Item 7** — Ambac Financial Group's FY2022 10-K (`0000874501-23-000040`) returned 538,701 chars for `obj['Item 7']`, roughly 70% of it Items 8, 9A and 10-15, and dropped Item 4 entirely. Its TOC runs Parts I-II down the left column and Parts III-IV down the right, interleaved one row at a time in the HTML, so a single running part context saw both columns' Part headers and items inherited whichever column last declared one — Items 7A-9B came out under Part I, Items 13-14 under Part II. Wrong Parts scramble the logical order that section boundaries are sorted by, which inverted three spans and left Item 7 unbounded. Columns are now read one at a time, and a Part header governs only its own column, with the right column continuing from the Part the left column ended in. Detection requires two *titled* links in opposite halves of a row, which deliberately excludes the label/title/page split (`[Item 1][Business][5]`) and headings fragmented across links (J&J's `P`/`art`/`I`) — a false positive here would discard the Part headers a 10-Q depends on, since its items repeat across Parts. Item 7 is now 158,411 chars and Items 3 and 4 are back. Section maps and section text are unchanged across all 55 fixtures. Reported by g-carmichael. (GH #924)

- **10-K item map emitting codes that do not exist in Reg S-K** — Foot Locker's FY2013 10-K (`0001144204-14-019510`) returned `Item 2P`, `Item 3L`, `Item 4M`, `Item 8C`, `Item 10D` and nine more from `.items`, each phantom sitting alongside the real item and splitting its content. TOC rows split the label and the title across cells (`<td>Item 4</td><td>Mine Safety Disclosures</td>`), and `text_content()` joins them with no separator, so "Item 4Mine Safety Disclosures" normalized to Item 4M — the suffix letter was the title's initial every time. A real suffix letter is never followed by a lowercase letter and a glued title's initial always is, so that now decides it; a genuinely glued suffix still resolves (`"Item 1ARisk Factors"` → Item 1A). Deliberately not a closed valid-set check, since filers do use company-specific suffixes. The phantoms had also corrupted the real items: `Item 1` returned 275,435 chars (nearly the whole filing) and now returns 3,340 opening on its own heading. Reported by g-carmichael. (GH #923)
