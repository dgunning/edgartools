...
- **`get_revenue()` returned a real number from the wrong year, and the wrong concept for insurers and banks.** Both came from one loop: `_get_standardized_concept_value` walked its `concept_variants` list in priority order and returned on the first variant with *any* match. That list ranks alternative *names* for one figure, so ranking by name before anything else produced two distinct wrong answers. NVIDIA migrated off `RevenueFromContractWithCustomerExcludingAssessedTax`, which stayed top-ranked, so `Company("NVDA").get_facts().get_revenue()` answered `26,914,000,000` — FY2022, four years stale — while `Revenues` carried the current `215,938,000,000`. Separately, MetLife tags both the ASC-606 slice (`2,436,000,000`) and consolidated `Revenues` (`77,084,000,000`) for the *same* FY2025, and priority took the slice: a 32x understatement across the sector, and an income statement showing $2.4B of revenue above $6.1B of operating income.

  Selection is now period first, then priority: the newest period any variant reports wins, and the priority list decides among the names that report it. Within that period, revenue additionally takes a magnitude cross-check — a candidate several times larger than the ranked pick is the consolidated total the pick is a slice of, not a rival name for it. Magnitude is only ever a cross-check on the ranked pick and never the ranking itself, since picking the largest outright would prefer `IncludingAssessedTax` over `Excluding` and gross over net. It is opt-in per concept family and off for everything else: net income's variants are not slices of one another, and preferring the larger `ProfitLoss` over `NetIncomeLoss` would quietly change whose earnings are reported.

  General Mills is the case that constrains the fix in both directions and is unchanged at `18,424,600,000`: its correct value is the *older-ranked* concept in the *newer* period, and its `Revenues` tag is a small slice, so neither "newest wins" nor "largest wins" would be safe alone.

  The same priority list exists a second time in the statement builder, and MetLife's income statement disagreed with `get_revenue()` for the same company and year. Both now share one threshold rather than two copies that can drift, and the revenue row is named after the concept its value actually came from instead of the highest-priority concept merely present in the filing. Every standardized getter built on this helper — `get_net_income`, `get_total_assets`, `get_total_liabilities` — was exposed to the staleness by construction and is fixed with it. (GH #1149)
