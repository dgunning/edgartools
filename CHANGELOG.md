...
- **`edgar_ownership` called a fund's unparsed positions "nan", and rendered the same 13F value two ways.** Both 13F walks serialise a pandas frame by hand, and each had grown its own coercion rules. A missing text cell arrives as NaN, which is truthy and which `str()` renders as the literal `"nan"`, so `fund_portfolio` answered `{"company": "nan", "cusip": "nan"}` for a position it could not name; `portfolio_diff` passed the bare NaN through instead, and `json.dumps` writes that as `NaN`, which is not JSON a strict parser will read back. One missing cell also makes its whole column float64, and the two walks disagreed about int coercion, so Berkshire's Apple stake came back as `65950296923.0` from `fund_portfolio` beside `65950296923` from `portfolio_diff`. Both now read cells through one pair of module-level helpers, which omit a missing cell and keep whole-number columns whole; genuinely fractional columns such as `share_change_pct` are unaffected. (Follow-up to the review on #1137)
