...
- **Trailing-twelve-month statements added up weighted-average share counts, reporting roughly four times the shares a company has.** EPS concepts were routed to a path that averages shares correctly, but everything else fell through to an unconditional sum over the trailing four quarters, so Alphabet's Q2 2026 TTM basic count came back 48,458,000,000 against an actual 12,114,500,000, and Tesla's 12,921,000,000 against 3,230,250,000. A share count already averages within its own quarter, so the trailing-twelve-month figure is the window's mean. The existing additivity check was *not* reused for this: it also excludes per-share amounts, and a dividend declared per share is a genuine flow — JPMorgan's trailing-twelve-month dividend is the 5.90 it declared across four quarters, not the 1.48 they average to — so only share counts and rates are averaged. Separately, both statement renderers printed a share count as an amount of money ("$12.1B"), because each carried its own copy of the per-share/currency rule and neither knew about share counts; the two classifiers now live once in `edgar.entity.unit_handling` and counts render as "12.1B shares". (bead edgartools-djwu)
- **A statement whose columns are not dates lost every column, so the frame came back with its rows and nothing else.** A column was kept only if its header matched a date pattern. A statement of shareholders' equity is broken down by equity component — `Total`, `Common stock and additional paid-in capital`, `Retained earnings/(Accumulated deficit)` — and carries the period in each row label instead, so no column was selected, every row was built with an empty value list, and `df['Total']` raised `KeyError` on a frame that still had all 47 of its rows. Everything but the line-item column is now treated as data when no header looks like a period, which is the rule the vertical-table branch already applied. Across ten filers' latest 10-Ks, 6 equity statements went from N×0 to N×4-8 columns carrying their filed values, and the other 57 statements were unchanged. Note this is not the defect the bead recorded: income statements were never affected, and two existing regression tests asserted that the fixture in question held no financial table — they were green because pandas reports a zero-column frame as empty, and both now assert the extraction. (bead edgartools-dhbg)
