...
- **`TenK.items`, `TenQ.items` and `TwentyF.items` no longer fall back to the deprecated legacy parser.** They now report exactly what the modern parser detects. This is not expected to change any result: across a 115-filing corpus spanning 1996 to 2026, removing the fallback changed the item list on zero filings, for every one of 10-K, 10-Q, 20-F and 8-K. The fix that made it redundant was Strategy 5c above, which recovered the last two filings that still needed it. Item *lookup* — `report["Item 7"]` and `get_item_with_part()` — still consults the legacy parser as a fallback, but no longer depends on it: it is reached whenever one particular item is missing rather than only when the whole document parsed to nothing, so a partial detection miss reaches it. Across the same corpus every one of those lookups is now answered by the modern parser — down from 15 `report["Item N"]` lookups and 4 part-qualified ones, via the wrapped-header, Part II, Item 9A(T), era-title, long-title, strategy-gate and part-aware-merge fixes below (and one lookup that was retired rather than fixed, where the legacy parser had been prefix-matching "Item 1112(b)" of Regulation AB when asked for Item 11, and returning nothing is the correct answer).

- **A fund's reference-data lookup now says when it failed, unless it failed by being offline.** `_build_hierarchy_from_mf_tickers` enriches series and class names from `get_fund_reference_data()` and falls back to the bare identifier when that is unavailable. The fallback is fine; catching every cause of it with `except Exception: pass` was not, because a network error, an SEC page restructure, a changed CSV shape and a bug inside `FundReferenceData` were all indistinguishable from "this class has no name" — and each produced a well-formed, plausible, wrong answer. That is how the redirect failure below stayed invisible for as long as it did. The handler now splits on `is_unreachable()`: genuinely offline degrades quietly, and every other cause degrades loudly with the exception type and message. Both halves matter, since warning on every offline call is how a useful warning gets tuned out. The returned value is unchanged either way, so nothing downstream shifts; this only decides whether anyone is told.

- **`get_latest_bdc_report_year()` no longer offers a hardcoded 2024 as though it were the latest year it found.** Both BDC probe loops ask whether a year or quarter exists by fetching a URL, and a year SEC never published answers 404, which `get_with_retry` returns as a response rather than raising — so reaching the `except` meant the probe had got no answer at all. Swallowing that made "we could not ask" indistinguishable from "SEC has nothing", and each loop then stated a conclusion it had not earned: `get_latest_bdc_report_year()` returned the hardcoded year, and `get_available_quarters()` returned `[]`, which reads as "SEC published none". Both now split on `is_unreachable()` and say which case they are in, with return values unchanged so no caller shifts; what changes is that falling back is stated out loud. Live, the year resolves to 2026, two years past the fallback. Alongside it, `edgar/funds/reference.py` and `edgar/bdc/reference.py` name SEC's migrated `data-research/sec-markets-data/*` addresses directly instead of paying a redirect hop, and a new network-marked contract suite watches the reference datasets themselves — ten assertions on specific values, among them that `C000013712` still resolves to "Advisor Class C", that `cik-lookup-data.txt` still matches `NAME:CIK:` across all 1,056,064 lines, and that AAPL is still CIK 320193 — each failure naming the source that moved, so a red run points at SEC rather than at the library. The repo's six existing contract files all pinned a filing parser; nothing had been watching the datasets.

### Fixed

- **`get_operating_cash_flow()` returned `None` for Apple, and took free cash flow down with it.** The method searched the cash flow statement for five label patterns — `^Net Cash from Operating`, `Net Cash.*Operations$` and three siblings — written against the *standardized* vocabulary. Apple labels the line "Cash generated by operating activities", which matches none of them, so the method returned `None` for every Apple filing, and `get_free_cash_flow()` returned `None` silently because it depends on this value. The filers it did work for worked by coincidence of house style: Microsoft writes "Net cash from operations", Coca-Cola "Net Cash Provided by Operating Activities", NVIDIA "Net cash provided by operating activities" — each landing inside one of the five by accident of wording rather than by anything the code arranged. Operating cash flow is now found by XBRL concept through the standardization store first, exactly as `get_revenue()` and `get_capital_expenditures()` already were, with the label patterns kept as a fallback for statements whose concept is absent or unmapped — a custom extension tag, or an IFRS filer outside the mapping set. Apple's latest 10-Q now returns $82,627,000,000, and the five issuers that already worked return the same figures as before. Reported in #1083, with a reproduction that measured the before and after.

- **Every fund series and class name came back as a bare identifier, and nothing said why.** SEC turned `about/opendatasetsshtmlinvestment_company` into a 301 whose `Location` is a bare path — `/data-research/sec-markets-data/investment-company-series-class-information` — which the standard expressly permits (RFC 7231 §7.1.2), and every manual redirect hop in `edgar.httprequests` passed that header straight into the next request. httpx received a URL with no scheme and raised `UnsupportedProtocol` before the request ever left the process, `edgar/funds/data.py` swallowed the failure and fell back to identifiers, and `find_fund("KINCX").name` returned `"C000013712"` instead of `"Advisor Class C"` — a well-formed, plausible, wrong answer that only two literal-value assertions in the fund tests happened to catch. `redirect_url()` now resolves the header against the URL that produced it via `httpx.URL.join` (RFC 3986 reference resolution), so an absolute `Location` passes through untouched and a protocol-relative one inherits the scheme, applied to all five hops: `get_with_retry`, `get_with_retry_async`, `stream_with_retry`, `post_with_retry` and `post_with_retry_async`. SEC is mid-migration from `about/opendatasets*` to `data-research/sec-markets-data/*`, so this closes the class rather than the one address.

- **A 10-Q's Part II items were judged against Part I's expectations, and told they were truncated.** The section size guardrail flags a section whose length falls outside what its item normally holds, attaching a warning and halving the section's confidence — but it keyed those expectations on the bare item number, and a 10-Q has two Item 1s: Financial Statements in Part I, around 90,000 characters, and Legal Proceedings in Part II, frequently a pointer of a few hundred. Part II's was measured against Part I's floor of 18,009 and reported as a likely truncated extraction on 18 of the 25 filings in the fixture corpus; counting Item 2 as well, 38 of the 65 size warnings the whole corpus produced were false alarms on correctly-extracted sections. A guardrail that is wrong more often than right is worse than no guardrail, because the silent wrong content it exists to catch gets ignored along with it. Bands may now be written per Part, and the 10-Q's are Part I's Items 1 and 2 and Part II's Exhibits, with the numbers unchanged — they were measurements of Part I all along. Part II's Items 1 and 2 are deliberately left unenforced: they run from 74 characters to 12,718 across the corpus, and no floor can separate "the filer said nothing" from "the anchor missed the body". Alongside it, the character count quoted in a warning now matches what `section.text()` returns, so the number a caller is given is one they can reproduce.
- **A section the filer answered with a cross-reference was reported as a truncated extraction.** The size guardrail flags a section whose content falls below its band and tells the caller the anchor probably landed on a heading — the right diagnosis when an extraction really did stop short, and the wrong one for an Item 8 that says the financial statements are filed under Item 15, where the extraction is faithful to the document and the content genuinely lives elsewhere. Every undersized Item 8 in the fixture corpus is one of these — NVIDIA at 207 characters, Netflix at 268, IBM at 250, Oracle at 158 and CIK 915358 at 112 — so the warning was misattributed in all five cases and not one of them was a truncation, which sent the caller off to debug a parser that had done its job. An undersized section is now tested for a deferral before the warning is written, and a pointer gets an incorporation-by-reference warning naming where the content actually lives. Both keep the reduced confidence, since a pointer is still not the item's substance: what a caller receives is unchanged, only what they are told about it. The text is extracted only for sections the bands already flagged, so a healthy filing does no extra work and the oversize path never pays for it. Reported in #927.

- **Two items of a 10-Q returned the identical text, with nothing to say which one was wrong.** Procter & Gamble's table of contents points its Item 5 and Item 6 rows at the same anchor in the body, so both keys sliced to the same 2,628 characters and Item 5's "Other Information" quietly answered with Item 6's exhibit list. The repair built for exactly this — Regions Financial's `Item 7A` returning Item 7's MD&A — re-points a colliding item at its own body heading, and here it had no headings to work with: P&G builds each one as a two-cell table row ("Item 5." beside "Other Information"), which renders with nothing between the number and the title, and the body scan required a space there, so it matched none of the filing's headings. A period may now stand in for that separator, though not when a digit follows it, which is what keeps an 8-K's "Item 5.02" from reading as a bare Item 5. Item 5 and Item 6 now return their own 306 and 2,320 characters; across a 115-filing corpus of every item-based form, this is the only filing whose sections change. Where a collision still cannot be separated, the warning now names what was done about it rather than only what was seen.

- **`get_item_with_part()` returned the wrong text on some 10-Qs, and it was two separate faults.** ExxonMobil writes six of its seven items as headings and puts Item 1 in a table; the strategy that reads item headers out of table cells only ran when fewer than half the form's items had been found, so it was held shut by the very headers that could never supply the missing one — and Part I Item 1 is the entire financial-statement section. Procter & Gamble's fault was different: a 10-Q has *two* Item 1s, Financial Statements in Part I and Legal Proceedings in Part II, and the code that fills in items a table of contents omitted compared bare item numbers, so a TOC that named one was read as having named both. In both cases the lookup fell through to an older extractor that answered with something far worse than nothing — 222,536 characters for a section of 975. Six sections across four filings are recovered, and nothing else moves: no section changed its boundaries.

- **An item whose title runs long was not recognised as a heading at all.** Header detection refuses to treat anything over fifteen words as a header, which is a reasonable guess for unlabelled text and a bad one for labelled text: several of the SEC's own canonical item titles are longer than that, Item 5's "Market for Registrant's Common Equity, Related Stockholder Matters and Issuer Purchases of Equity Securities" among them at seventeen words. The cap therefore rejected precisely the longest *real* headers. On one 2016 annual report Item 5 was the only item of twenty that went missing, and `tenk["Item 5"]` returned text only because the deprecated legacy parser rescued it. A filing's own "Item N" or "PART N" label now waives the length test, while unlabelled long text and prose cross-references are treated exactly as before.

- **Older 10-Ks lost Items 4 and 14, because each item has carried two titles and only the modern one was recognised.** Item 4 was "Submission of Matters to a Vote of Security Holders" until 2011, when Dodd-Frank gave it to mine safety; exhibits were Item 14 until the 2003 renumbering moved them to Item 15 and gave Item 14 to accountant fees. A pre-2011 annual report's Item 4 header and a pre-2003 Item 14 header were both found and then discarded for having the wrong title, so `tenk["Item 4"]` and `tenk["Item 14"]` returned text only because the deprecated legacy parser rescued them. Both older titles are now recognised alongside the modern ones, and a modern filing's Items 14 and 15 are unaffected.

- **An item that numbers its own subsections no longer stops at the first of them.** A filer who divides Item 14 with "Item 14(a)(1):", "Item 14 (a)(2):" and "Item 14 (a)(3):" headers was handing the section extractor three things that look exactly like item headers, and the item ended at one — on a 1999 annual report that returned the financial-statement list and dropped the schedules and the whole exhibit index. An item marker carrying a parenthesized sub-designation and no title is now read as a subdivision rather than a new section. Related: a bare "SIGNATURES" line now ends a section regardless of how the heading detector scored it, so a report's last item no longer runs past the signature page to the end of the document — seven sections across 10-K, 10-Q, 20-F and 8-K stop where they should, among them Item 16 on two 2021 annual reports and Item 19 on a 20-F.

- **`Item 9A(T)` was unmatchable, so a cohort of 2007–2010 annual reports had no controls-and-procedures section.** `9A(T)` was the SEC's transitional designation for a smaller reporting company's internal-control report, and a filer writing `ITEM 9A(T). CONTROLS AND PROCEDURES` hit an item-header pattern whose punctuation vocabulary had no `(` in it — so the match died right after the number and the section was never created, though the header itself was found. `tenk["Item 9A"]` returned text only because the deprecated legacy parser rescued it. An item number may now carry a parenthesized designation, across 10-K, 10-Q and 20-F alike.

- **A 10-Q whose "PART II" marker is styled on an inner element lost its whole Part II.** Some filers — Goldman Sachs among them — render the marker as a paragraph that carries no styling itself, wrapping a bold span. The section extractor only recognised that shape for 10-K and 8-K, and the consequence was larger than one missing marker: each item header is assigned to the last part seen, so with no Part II boundary every later header was still Part I, and the Part II patterns then rejected their own headers. Items 5 and 6 were being found and thrown away. `tenq["Item 6"]`, `tenq["Item 5"]` and both `get_item_with_part("Part II", …)` calls returned text only because the deprecated legacy parser rescued them; on the same filing, `get_item_with_part("Part II", "Item 6")`'s *other* fallback returned nothing at all. A bare "SIGNATURES" line is now recognised on 10-Q as well, which is what stops Item 6 (Exhibits) running past the end of the exhibit list — three further filings gained a correctly bounded Item 6 from that, ExxonMobil's among them.

- **Item headers that wrap across two lines are now found.** A filer whose HTML reads `<td>ITEM 5. OPERATING` / `AND FINANCIAL REVIEW AND PROSPECTS</td>` hands the section extractor a header with a newline in the middle of its title. In HTML that newline is just whitespace, but the section patterns join words with `.*`, which does not cross one — so which items a filing lost was decided by an accident of how each pattern happened to be written: Item 4's pattern is built from `\s+` and matched, Item 5's uses `.*` and did not. Header text is now normalized before matching. On a 2010 20-F this recovers Items 5, 6, 11, 12, 15 and 16D-16F — `twentyf["Item 5"]` returned 107,307 characters only because the deprecated legacy parser rescued it — and on a 2016 20-F, Items 6 and 11; on a 1999 10-K, Item 7A. Alongside it, a table-of-contents row can no longer outrank the real body header for the same item when it happens to span more of the document.
