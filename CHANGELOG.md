...
- **A cash flow statement selected by its role URI exported its capital expenditures positive, where the same statement selected by name exported them negative.** The presentation-sign gate keys on a statement's type, and `Statements.__getitem__` inferred that type by testing whether a canonical name appeared inside the string — which is never true of a role URI, since no issuer writes "CashFlowStatement" into `http://www.apple.com/role/CONSOLIDATEDSTATEMENTSOFCASHFLOWS`. Every role-selected statement therefore carried no type, and a correctly hydrated `preferred_sign` of -1 was skipped: Apple's FY2023 capex came back +10,959,000,000 from one accessor and -10,959,000,000 from the other. The type is now read back from the resolver's own classification, through a new `Statement.classified_type` kept deliberately separate from `canonical_type` — that attribute also decides *which* role `render()` renders, so populating it turns "render this role" into "render the canonical statement of this kind", which on Boeing dropped a filed $24,000,000 asset impairment charge from the cash flow statement. Across the fixture corpus 1,060 rows in 35 statements gain their sign, none change magnitude. (GH #1285, bead edgartools-vaau.1)
- **`render(standard=False)` displayed cash-flow outflows as positive, disagreeing with its own DataFrame.** `statement_type` was written onto the row dictionaries only along the standardization path, so with `standard=False` the cell formatter received rows with no type and passed `None` into the sign gate. The `RenderedStatement`'s own `statement_type` stayed correct, which is why `to_dataframe(presentation=True)` returned -10,959,000,000 while the formatted cell beside it read `$10,959`. Whether a row belongs to a cash flow statement has nothing to do with whether its label was standardized, so the type is now stamped on every row regardless of the flag. (GH #1289, bead edgartools-vaau.2)
