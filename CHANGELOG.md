...
- **A filing that failed to parse vanished from a stitched `XBRLS` with no trace.** `XBRLS.from_filings()` swallowed every exception, so a batch that lost a filing looked identical to a complete one. The failure is now logged as a warning naming the accession number, form and exception; a filing with no XBRL at all is still skipped quietly. (GH #1174)

## [5.57.0] - 2026-09-08

### Fixed

- **Selecting a statement by its role URI could return a different statement.** Any role whose name mentioned a statement was typed as that statement, so Apple's balance-sheet parenthetical rendered the primary balance sheet. Known roles now take the resolver's classification; 166 of 2,658 corpus roles change and no primary statement loses its signs. (bead edgartools-vaau.3)
- **`aggregate()` summed the filed strings and raised `TypeError` on ordinary numeric facts.** It now aggregates `numeric_value` and skips facts that have none; Apple's FY2023 product revenue by `srt:ProductOrServiceAxis` returns totals. (GH #1277, bead edgartools-n93w.3)
- **`by_concept()` could not match a concept whose name contains an underscore.** Every `_` was rewritten to `:`, so YUM's `yum:YUM_LesseeOperatingLease…` matched nothing in either spelling. The pattern is now matched against both spellings unaltered; 20,094 exact queries across 15 filings lose no row. (GH #1276, bead edgartools-n93w.4)
- **`facts_history(include_dimensions=True)` returned the undimensioned series.** The flag never reached the query. It does now, and the date column falls back to `period_instant` for instant facts — previously every balance-sheet concept returned an empty frame. (GH #1286, bead edgartools-n93w.5)
- **Sorting one fact query reordered the cache every other query reads from.** `sort_by()` sorted the shared fact list in place, so an unrelated `.limit(5)` changed on 15 of 15 filings measured. The sort now builds a new list. (GH #1287, bead edgartools-n93w.1)
- **`sort_by()` raised `TypeError` on most filings, and silently did nothing on a sparse column.** `None` values broke the comparator, and the column was looked for in the first row only. The key is now a total order over mixed types and the column is found anywhere in the result. (GH #1288, bead edgartools-n93w.2)
- **`analyze_trends()` returned `{}` for a 10-K reporting exactly two annual periods.** It required a named three-period view; it now falls back to the periods the statement displays, so Auburn National's $603,000 and $598,000 of contract revenue are reported. (GH #1292, bead edgartools-hsgs.3)
- **A ratio could divide one period's number by another's.** `calculate_ratios()` let each operand take the first value in its own dictionary, so Netflix's Q3 2024 net margin was 0.2767 against a correct 0.2406. Every operand now comes from one period, and a ratio missing an operand is omitted rather than borrowed. (GH #1280, bead edgartools-hsgs.1)
- **Synthetic totals added a statement's subtotals to their own components.** Disney's beginning equity was reported as $292.8B against a filed $98.9B. The filer's own total member is now preferred and only a genuinely disjoint breakdown is summed; 20 values change corpus-wide. (GH #1281, bead edgartools-hsgs.2)
- **`RenderedStatement.to_dict()` produced output that was not JSON.** A prior-period value of zero yielded `Infinity` in the comparison, which 17 corpus filings hit. Non-finite comparisons now serialize as `null`. (GH #1290, bead edgartools-hsgs.4)
- **An axis with two domain roots kept only the first, and a childless root disappeared from the dimensional model.** Ellington Financial's `efc_Year2038Member` was absent from `domains_for_role()` though five facts use it. Every root is now registered; `Axis.domain_id` still names the first. (GH #1278, bead edgartools-xn1u.2)
- **Every iXBRL date written with an English month name was rejected unless its fields were separated by a space.** `19-Sep-2024` failed all thirteen word-date formats. The tokenizer now accepts the registry's separators; 40,219 facts across 16 filings show no already-transformed value changing. (GH #1284, bead edgartools-xn1u.1)
- **A cash flow statement selected by its role URI exported its capital expenditures positive, where the same statement selected by name exported them negative.** Role-selected statements carried no type, so the sign gate was skipped. The type is now read back from the resolver; 1,060 rows in 35 corpus statements gain their sign, none change magnitude. (GH #1285, bead edgartools-vaau.1)
- **`render(standard=False)` displayed cash-flow outflows as positive, disagreeing with its own DataFrame.** The statement type was stamped on rows only along the standardization path; it is now stamped regardless of the flag. Apple's FY2023 capex reads −10,959 in both. (GH #1289, bead edgartools-vaau.2)
- **`get_total_liabilities()` returned liabilities *and equity*, so `debt_to_assets` came back as exactly 1.0.** A `Liabilities$` label pattern matched the combined total; the getter now identifies the concept and returns `None` where no consolidated total is filed (Coca-Cola, Amazon). NIKE's FY2026 ratio goes from 1.0 to the correct 0.613. (GH #1279, bead edgartools-y4yp.1)
...
- **`get_shares_outstanding_basic()` returned `None` for shares present in the statement.** The lookup stopped at the abstract heading above the populated row; it now skips abstract rows and iterates its matches, returning Auburn National's 3,498,030. (GH #1291, bead edgartools-y4yp.3)
- **Trailing-twelve-month statements added up weighted-average share counts, reporting roughly four times the shares a company has.** Alphabet's Q2 2026 TTM basic count was 48.5B against an actual 12.1B. Share counts and rates are now averaged over the window; flows such as dividends per share are still summed. Both renderers also now show counts as "12.1B shares" rather than "$12.1B". (bead edgartools-djwu)
- **A statement whose columns are not dates lost every column, so the frame came back with its rows and nothing else.** Equity statements broken down by component matched no date header and returned N×0 frames. Non-date columns are now kept as data; 6 of 63 statements across ten filers change, the rest are identical. (bead edgartools-dhbg)
- **A filed context whose ID ends in `_1` collided with an internal duplicate-occurrence index, and the parser silently dropped one of the two facts.** ClearOne's Q2 2025 10-Q lost 6 of 590 facts, including the quarter's own revenue. Key parts are now joined with `|`, which no NCName admits. (GH #1295, bead edgartools-jj77.1)
- **A concept whose name merely ended in `unit` was discarded as a structural element.** `ctso:NumberOfSharesInAunit` was skipped by a bare-suffix test; structural elements are now matched by fully qualified name. (GH #1293, bead edgartools-jj77.2)
- **Fact DataFrames deleted a fact denominated in a different currency when its number happened to match.** Aebi Schmidt's CHF 10,000,000 and EUR 10,000,000 loans collapsed to one row. The unit is now part of the duplicate key; no frame in the 33-filing corpus changes size. (GH #1282, bead edgartools-jj77.3)
- **Stitched fact DataFrames deleted whichever columns the matched rows left empty, so narrowing a query changed the shape of the table.** `by_statement_type('BalanceSheet')` lost `period_start` on 5 of 5 stitched pairs. Columns and dtypes now follow the query's configuration per `engineering/decisions/facts-dataframe-schema.md`. (bead edgartools-trhe)
- **`pivot_by_period()` on company facts discarded facts that collided on a key too narrow to tell them apart, and for an IFRS filer discarded nearly all of them.** IFRS concepts carry no label upstream, so 765 of LPA's 768 facts shared one row. Unit is now part of fact identity and the pivot widens its key only for rows that collide; the filing pivots to 188 rows. (GH #1196, bead edgartools-6byc)
- **Stitched fact rows named no source filing, and every row reported the newest filing's fiscal focus.** `source_filing_index` was always `None` and `by_filing_index()` matched nothing. Rows now carry the filing the stitcher selected each period from; a two-filing ORCL stitch splits 115/100. (GH #1205, bead edgartools-qbm7)
- **The entity fact DataFrames changed shape and dtype according to which rows a query matched.** `value` flipped `float64`→`int64` on 16 of 36 narrowing queries, and an empty result had no columns. `FactQuery.to_dataframe()` and `EntityFacts.to_dataframe()` now declare their columns and dtypes; `fiscal_year` stays `int64` until 6.0. (bead edgartools-7wtj)
- **Line-item tags resolved to `Total*` concepts, so a standardized statement returned several rows all claiming to be the same total.** Citizens Financial's balance sheet had three "Total Assets" rows. A tag not marked `is_total` no longer resolves to a total (74 spellings), and IFRS tags answer only from IFRS entries. (GH #914, bead edgartools-gnx5)
- **`render(standard=True)` returned a DataFrame with no standardization in it.** The `standard_concept` column was never emitted. It is now present for every standardized statement and absent otherwise; filer labels are unchanged. (GH #1083, bead edgartools-t3zh)
- **A statement row could report a different concept's number, and the row that owned it was deleted.** `_merge_complementary_rows()` matched dimensional rows on label alone, merging 213 different-concept pairs across the corpus. A dimensional row must now also match on concept; 2 merges remain. (GH #1206, bead edgartools-j7iz)
- **Every per-share unit was parsed as plain currency, discarding its denominator.** `xbrli:divide` was unreachable, so `usdPerShare` was recorded as USD. Divided units are now parsed, and the unit-to-currency rule lives once in `edgar.xbrl.core.unit_currency`. (GH #1170, bead edgartools-uetp)
- **A presentation role's second root, and every fact beneath it, was missing from the statement.** Union Pacific's FY2012 leases note returned 13 of 22 nodes. `PresentationTree` now carries `root_element_ids`; 9 of 2,052 corpus roles are multi-root and gain 106 rows. (GH #1245, bead edgartools-0q0d)
- **A concept presented twice in one role reported the last occurrence's parent and indentation on both rows.** Rows now take their position from the tree walk rather than the shared node; 77 rows' parent and 71 rows' level are corrected across the corpus. (GH #1231, bead edgartools-f07v)
- **A negated label rendered with the filed positive sign, and on the Facts API nothing was ever negated at all.** Legacy `xbrl.us` roles matched neither of two independent matchers, so Apple's FY2010 securities purchases showed +57,793M. Both paths now use `is_negated_label_role`; Apple FY2023 goes from 0 to 130 negated facts. (GH #1249, bead edgartools-hxnw)
- **`Statements.to_dataframe()` and `Statement.to_dataframe()` returned different signs for the same line.** The sign now lives in one function called by both; 13 rows of Apple's FY2023 cash flow change. `to_dataframe(presentation=False)` returns the raw filed values. (GH #1248, bead edgartools-5ztr)
- **`get_structured_statement(fiscal_period='FY')` could return the prior-year comparative.** A 10-K tags every instant `fp=FY`, and the filing-date tiebreak never fired. Period end now breaks the tie; Apple's FY2023 balance sheet reports `AssetsNoncurrent` 209,017,000,000 instead of the FY2022 217,350,000,000. (bead edgartools-i24a, GH discussion #946)
- **Period and statement selection silently ignored what the caller asked for, in three places.** `select_periods(max_periods=4)` returned 10 periods; balance-sheet views admitted empty incidental instants; `parenthetical=True` returned the face statement. The cap is applied, instants are fact-checked, and a filing with no parenthetical role raises `StatementNotFoundError`. (GH #1221, #1222, #1246, beads edgartools-rj4b, edgartools-zt8u, edgartools-z0y2)
- **Three public `Statement` helpers had never returned a correct result for any filing.** `analyze_trends()` read a key that was never emitted, `calculate_ratios()` looked up reversed concept names, and `to_dataframe(matrix=True)` read every cell from `standard_concept`. All three now return filed values; Apple's FY2023 equity matrix goes from 33 null cells to its filed balances. (GH #1240, #1241, #1244, bead edgartools-ysr8)

### Performance

- **`get_financial_metrics()` rendered and converted the same three statements once per metric.** The three getter helpers share one `_render_statement_frame()` with a per-call memo, so a call renders 3 times instead of 14–18. JPMorgan's warm median drops from 95.2 to 24.8 ms with every metric byte-identical. (GH #1283, bead edgartools-y4yp.4)
