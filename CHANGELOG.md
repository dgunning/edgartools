...
- **`ParagraphNode.text()` no longer spaces adjacent inline elements on the strength of their tag name.** The per-tag allowlist (`original_tag in span/a/em/strong/i/b`) that the five fixes below were built to make redundant is removed, and a third boundary signal replaces it: a standalone list or checkbox glyph in the text (`• Proposal No. 1`, `* Certain projects`, `o Yes`), alongside the existing CSS-gap and fixed-width-marker-box tests. Unlike those two it reads the text rather than the style, which is what reaches SigmaTron's cover page — the filer writes `style="…font-family: "Wingdings""`, nesting double quotes inside a double-quoted attribute, so `font-family` parses as empty for us and for a browser alike and no style-based rule can fire there. Letter markers (`o`, `ý`, `þ`, used as Wingdings checkboxes) are additionally guarded against a following lowercase run, because A-Power's FY2009 20-F splits `our` as `o`+`ur` across two elements and would otherwise extract as `o ur wind turbine business`.

  The allowlist had been kept because deleting it appeared to cost 8,109 spaces. That figure was a measurement artifact: the harness scored every removed space as "lost", so a word *repair* counted identically to a destroyed boundary. Classifying each removal against the filing's own vocabulary — a join is a repair if the joined token is spelled whole elsewhere in the same document — inverts the result. Of the 245 spaces the removal changes across the 57 large-cap fixtures, **222 are confirmed repairs**: 92 a space wrongly inserted after an opening quote or paren (`(the " SEC")`, `( i.e.,`, `http:// www.sec.gov`), 105 a word the allowlist itself split, and 25 the corruption of Item headings it caused directly — `Item 1A. RI SK FACTORS`, `ITEM 1B. UNRESOLV ED STAFF COMMENTS`, `ITEM 2. PR OPERTIES`, `ITEM 7A. … QUALITAT IVE`, which is exactly the string a section matcher keys on.

  Verified on the 57 fixtures and on a new 129-filing corpus spanning five markup eras and nine form types (10-K, 10-Q, DEF 14A, S-1, 424B2, N-CSR, N-PX, 20-F, 8-K), built because the fixture corpus turned out to be unrepresentative on tag vocabulary and not merely on era: its median `<span>` count is **0 in nearly every era/form group including 2020-2026**, since filing agents still emit `<font>` with `<b>`/`<i>`/`<a>` inline, and 59 of the 129 documents cannot exercise the allowlist at all. Fixtures: 35 of 57 change, +11 spaces, −209, **no non-whitespace content changes**, section map 0 of 55. Wide corpus: 16 of 129 change, **+340 spaces**, −46, no content changes. Deleting the allowlist with no replacement was measured and rejected — it destroys 63 real boundaries on the wide corpus, 43 of them bullets in one small-cap 2021 proxy. Residual known losses: two adjacent inline elements carrying no signal at all are now joined (2 occurrences on the wide corpus, pinned in `TestMidWordSplitSpacing`), and six image filenames leaking into Mastercard's FY2024 10-K text, which is a separate defect (edgartools-j9mi). The now-unused `_splits_a_word`/`_same_typeface` helpers are removed; `Style.font_family` is retained as descriptive CSS metadata. Parse time on the three largest fixtures is unchanged (−1.4% to −0.2%).

  The five pinned `Filing.text()` baselines are re-captured — the first time in this family that a hash moves because output *loses* a space rather than gains one. 2 of the 5 filings change: Apple −1 char (`“ The Company’s operations` → `“The Company’s operations`) and 10x Genomics −5 chars across 5 lines (`“ Risk Factors - R…` → `“Risk Factors - R…` three times, which is the exact string its Item 1A cross-references, plus `Customer’ s Accounting` → `Customer’s Accounting`). Merck, Exelon and the BofA 424B2 are byte-identical. Both changed filings satisfy `"".join(before.split()) == "".join(after.split())`, and all four distinct repairs were read individually — no destroyed boundary. (edgartools-jysx)

### Fixed

- **Bullets and bare enumerators promoted to headings** — header detection scored a short styled span on everything except what it said, so a filer who put a bullet glyph in its own span got a `HeadingNode` out of it. `ContextualDetector` awards +0.3 whenever the next element is three times longer, a test a one-character text passes against almost anything, and another +0.3 when the *previous* sibling looks like a heading, which is enough for a bare `•` to clear the 0.6 threshold on borrowed evidence alone. Across the 57 fixtures that produced 17,016 `HeadingNode`s of which **2,818 contained no alphanumeric character at all** (2,636 of them a bare `•`, plus `®`, `*`, `**`, `—`) and **1,199 were bare enumerators** (`(1)` ×203, `(a)` ×134, `(2)` ×124) — together 23.6% of every heading built. Meta's FY2024 10-K was 180 of 296, JPMorgan's 10-Q had 147 enumerators, and Apple's FY2024 10-K returned 12 bullets and 8 `®` from `doc.headings`. They reached users through `doc.headings`, through the markdown renderer as `### •` and a matching table-of-contents entry, and through the heading index `DocumentSearch` scores.

  `HeaderDetectionStrategy.detect()` now applies a content floor before any detector runs: text with no alphanumeric character, or text that is nothing but a list or footnote enumerator, is not a heading however it is styled. The floor rejects on content rather than on tag name — adding `span` to `skip_header_detection_tags` was the tempting fix and would have lost the real headings that filers do mark with a styled span. Measured after: 12,999 headings with **0 of either kind remaining**, section map **0 of 55 fixtures changed**, `doc.text()` **byte-identical on all 57 fixtures**, and no markdown token lost once escaping and emphasis are normalised — the demoted markers reappear as inline paragraph content (`\(1\)`, `**•**`, `<u>—</u>`) rather than as heading text, and three stray mid-line `###` artifacts disappear. The five pinned `Filing.text()` baselines are unchanged, so no re-capture was needed.

  This is the defect the CSS-gap rule below was covering for: a `HeadingNode` carries no `original_tag`, so the boundary fix had to read the style instead. Demoting the glyph to an inline text node is only safe now that `_is_bare_marker` exists to keep `• iPhone` apart, which is pinned in `TestHeadingContentFloor`. Two further defects found while measuring this one are filed rather than fixed here: `PatternBasedDetector` applies `re.IGNORECASE` to patterns whose discrimination *is* the case class, so its "All caps headers" rule matches any lowercase text and promoted the bare word `or`; and `ContextualDetector` still promotes mid-sentence prose fragments (`, Mac Studio`, `The Company offers a portfolio of…`), 120 remaining across the fixtures. (edgartools-1xxo)
- **Words glued together across inline-element boundaries** — the HTML preprocessor collapsed whitespace at text↔tag boundaries by *deleting* it, so `<font>UNITED STATES </font><font>SECURITIES...` extracted as `UNITED STATESSECURITIES`: the word boundary was destroyed in the HTML string before lxml ever parsed it, unrecoverable downstream (a per-tag allowlist in `ParagraphNode.text()` papered over `span`/`b`/`i`/etc. but not `font`, and could not distinguish a destroyed boundary from a genuine mid-word split). Ubiquitous in font-tag-era filings and present in current ones too — Morgan Stanley's FY2024 10-K alone had ~490 glued boundaries (`TheFederal Reserve`, `totaled$10.7 billion`, `theChief Information Officer`). Boundary whitespace is now collapsed to a single space, never deleted, matching HTML rendering semantics; genuinely unspaced splits (`<font>Edgar</font><font>Tools</font>`) stay glued. Blast radius measured across all 57 HTML fixtures: every document changes by +3 to +493 chars (~0.01–0.03%), all restored word boundaries; section detection is unchanged (0 of 55 fixtures). (edgartools-tlj1)
- **A word gap lost when the whitespace sat inside a wrapper** — a fifth copy of the delete-vs-collapse bug, and one the two fixes before it created. `DocumentBuilder` records a whitespace-only tail as metadata on the element that owns it, which is the innermost one, but `ParagraphNode.text()` read that flag off the sibling it was comparing — so it was missed whenever the whitespace sat one or more levels down. Chevron's FY2024 10-K puts a run-in heading in an `<ix:nonNumeric>` with the gap after it in a spacer span *inside* that element, and shipped `GeneralThe Company follows`; SigmaTron's FY2025 10-K shipped `New Accounting StandardsIn June 2016`. This only became reachable once the preprocessor started turning a spacer element into whitespace and lxml stopped deleting it — before those two fixes there was no whitespace-only tail left to lose. The flag is now looked for down the rightmost spine. Landed with a second signal that changes nothing on its own but is needed to retire the tag allowlist: a marker in a fixed-width box (`<span style="display:inline-block;width:0.250in">-</span>`, how SigmaTron lays out its risk-factor bullets) draws the same gap as a `padding-left` on the text, from the other side, and is now read the same way. Measured across all 57 HTML fixtures: 2 change, **7 lines gain a space, 0 lose one, no non-whitespace content changes**; section map 0 of 55. (edgartools-jysx)
- **Bullets and footnote markers glued to their text** — Apple's FY2024 10-K shipped `•MacBook Pro 16-in.; and`, AbbVie's Item 15 shipped `(1)Financial Statements:`, and 640 more across the fixture corpus. A filer who puts a bullet glyph in one span and the item text in the next does not separate them with whitespace: the text span carries a `padding-left`, and that CSS gap *is* the word boundary. `ParagraphNode.text()`'s tag allowlist had been standing in for it — 95% of the 8,109 boundaries the allowlist restores across the corpus turn out to carry such a gap — but the allowlist keys on `original_tag`, metadata only text nodes carry, and header detection promotes a short span like a bare `•` to a heading node, which loses it. The gap is now read off the style directly, independent of node type. Measured across all 57 HTML fixtures: 40 change, **640 lines gain a space, 0 lose one, no non-whitespace content changes**; section map 0 of 55 changed. The heading-node promotion itself is a separate defect (a `•` is not a heading, and `doc.headings` returns 14 of them for Apple) and is filed rather than fixed here. (edgartools-jysx)
- **Words split mid-token by the spacing heuristic that the four fixes below exist to make redundant** — `ParagraphNode.text()` force-spaces adjacent inline elements, because at that point nothing can tell a boundary an upstream pass destroyed from a word the filer split mid-token. Filers do split words mid-token, and force-spacing those shipped `designed to identify, asse ss, and monitor` in Apple's FY2024 10-K (source: `asse</span><span style="background-color:#ffffff">ss`), plus 225 more across the fixture corpus (`Th e facility`, `jurisd ictions`, `Chevr on`, `approxim ately`). The insertion is now suppressed where it would weld two lowercase fragments set in the same typeface with no CSS gap between them — the two signals that mark a real word gap being a `padding-left`/`margin-left` on the second fragment (filers draw gaps that way) and a change of font family, which is how SEC cover-page checkboxes (`Yes ☒ No ☐`, glyphs in Wingdings) stay separated from their labels. Requires the new `Style.font_family`. Measured across all 57 HTML fixtures: 225 lines change, 261 spaces removed, 202 distinct word repairs, **no line's non-whitespace content changes**; every repair was reviewed and none welds two real words. The allowlist itself stays — deleting it outright still costs 8,109 spaces on the same corpus, so this narrows its false positives rather than replacing it. (edgartools-jysx)
- **Words glued by lxml deleting the space the preprocessor had just saved** — a fourth copy of the same bug, and the one underneath the other three: the parser passed `remove_blank_text=True` to lxml, and libxml2 discards a text node it judges to be all-ignorable whitespace. The preprocessor deliberately collapses a whitespace run between two tags down to *one space* because that space is a word boundary — and then that lone space, now a whitespace-only text node, was deleted anyway. JPMorgan's FY2024 10-K note 32 read `the Firm has threereportable business segments`, SigmaTron's FY2025 10-K read `Non-accelerated Filerý` and `non-Rule10b5-1 trading arrangement`, and the `Yes ☒ No ☐` cover-page checkboxes on Apple, AbbVie, Amazon, Alphabet, Meta, Pfizer, Visa and a dozen more read `Yes☒`. The losing shape is a whitespace-only *tail* on a child of an inline element, which is why three earlier passes over this bug family missed it: for JPMorgan and Boeing that child is an `<ix:nonFraction>` fact, a tag libxml2 has never heard of, so it treats the tail as ignorable. Blank text nodes are now kept and collapsed downstream like every other boundary. Measured across all 57 HTML fixtures: 27 change, 83 lines gain a space, **0 lose one, and no fixture's non-whitespace content changes**; parse time on the three largest fixtures is unchanged (−3.1% to +0.7%, i.e. noise). (edgartools-vfwp)
- **Words glued by deleted spacer elements** — a third copy of the same bug, in `_remove_empty_tags`: a tag holding only whitespace was treated as empty and deleted outright, whitespace and all. Filers draw a word gap that way, with a styled spacer element sized to the gap they want — Apple's FY2024 10-K separates "the App Store and Safari" from "in the EU" with `<span style="font-size:5.85pt"> </span>` — so the words either side were glued. It read `Washington98052-6399` on Microsoft's Q3 cover page, `Chevron holds a63 percent-owned`, `Kazakhstan/RussiaChevron has a 15 percent interest`, `Yes☒ No☐` on Adobe's cover page and `☒ANNUAL REPORT` on Apple's. A tag containing whitespace now leaves a single space behind; a genuinely empty one still leaves nothing. Measured across all 57 HTML fixtures: 41 change, 171 lines gain a space, **0 lose one, and no fixture's non-whitespace content changes** — four filings re-wrap a table header row because a cell gained a character, which shifts column widths without altering content. Note this bug was largely invisible before the two fixes above: `ParagraphNode.text()`'s allowlist heuristic force-spaces adjacent spans whenever the following character is a letter, so only gaps before a digit or a glyph showed through. (edgartools-jysx)
- **The same word gluing, one layer down** — fixing the preprocessor left a second copy of the bug in `DocumentBuilder._process_element`, which built text nodes with `element.text.strip()` / `element.tail.strip()`. lxml puts precisely the word-separating whitespace on those edges, so the boundary was destroyed again before `ParagraphNode.text()` saw it; that method then *guessed* it back from punctuation plus a hardcoded `['span','a','em','strong','i','b']` allowlist, which is why `<font>` filings stayed glued while `<span>` ones looked fine. Airbnb's S-1 read `an accelerated filer, anon-acceleratedfiler`, `synonymous withone-of-a-kindtravel`, `asCOVID-19disrupted travel`; 10x Genomics' FY2019 10-K read `held bynon-affiliatesas of such date` and `Annual Report on Form10-Kwhere indicated`; Bank of America's 424B2 read `$1,246.00 per$1,000 in principal`. Edge whitespace is now collapsed to a single space, never deleted, matching the preprocessor rule — genuinely unspaced edges (`Regulation<font>S-T</font>`) stay glued. Blast radius measured across all 57 HTML fixtures: 2 change (the 2 that use `<font>`), by +980 and +989 chars; a character-level classifier confirms every changed line differs by inserted spaces only, with zero content changed and no space removed anywhere; section detection unchanged (0 of 55 fixtures, 0 sections lost or gained). Table cells were never affected — they take a separate assembly path that already preserved tails. (edgartools-tlj1)
- **Documents over 10MB parsed by a second, lossy pipeline** — `HTMLParser` routed anything above `ParserConfig.streaming_threshold` (10MB) to a separate `StreamingParser` that only materialised `p`/`h1-h6`/`section`/`table` nodes. Text hosted directly in `<div>`s — which is *most* of the text in a modern SEC filing — went to a buffer that was never flushed: on the 16.7MB Citigroup FY2024 10-K fixture, `text()` returned 830K chars where the normal pipeline returns 1.81M — **54% of the document silently missing**, along with the MD&A, Risk Factors, Financial Statements and Controls sections. The streaming path also skipped the HTML preprocessor (so `&nbsp;` survived as `\xa0`, encoding fixes never ran) and returned before inline-XBRL extraction, and it was *slower* than the pipeline it was meant to relieve: 61s vs 2.4s on the same 15MB document, superlinear above that — an 88MB 2007 N-PX that ran 90+ minutes without finishing now completes in 39s at 2.3GB peak RSS. Its memory saving was marginal (0.25GB vs 0.40GB on 15MB) because it built a full document model anyway. `StreamingParser` is removed; every document now takes the one lxml pipeline, bounded by `max_document_size` (160MB default). `streaming_threshold` remains on `ParserConfig` as a deprecated no-op so existing call sites keep constructing. Which pipeline a user got — and therefore what text a corpus contained — was previously decided by nothing more than whether the filing crossed 10MB. Reported from the same 7M-filing crawl as the ABS-15G fix below.
...
- **`text()` on very large tables taking hours** — `TableMatrix._calculate_dimensions` asked `_is_occupied()` whether each position was covered by a rowspan from above, but it runs *before* `build_from_rows` materialises the grid, so the answer was always `False`, reached by scanning an empty structure once per preceding row: O(rows²) of pure dead work. On Fannie Mae's `0000310522-18-000010` (25 MB, 61,801 rows, 1,483,203 cells) that was 192.5M length checks, and `FilingSGML.text()` took **1h12m — now 24.1s**, with byte-identical output (SHA-256 verified at 500/2,000/6,000 rows; `section_map_diff` reports 0 of 55 fixtures changed). `build_from_rows` now also resets the grid, which was a latent correctness bug: a reused instance read the previous build's rowspans as if they described the new rows, widening a 2-column table to 3. Reported from a 7M-filing crawl where 425 of 428 long-runtime warnings were ABS-15G filings.
- **XBRL extraction warning untraceable, and breaking warning interception** — `Failed to extract XBRL data` was logged with the module-level `logging.warning`, i.e. the root logger (`WARNING:root:`), carrying no document context at all — untraceable in a bulk crawl. It was also called from inside `HTMLParser.parse()`'s `try:`, whose `except Exception` re-raises as `HTMLParsingError`; a `logging.Handler` that raises on WARNING (the standard way to locate a warning) therefore had its exception swallowed and converted into a parse failure, so the warning vanished entirely. The step is best-effort and already swallows its own errors, so it now runs outside that wrapper. It logs to `edgar.documents.parser`, lxml's benign "Document is empty" drops to DEBUG (a document with no parseable root has no inline XBRL either), and genuine failures carry the exception type, size and a content preview.
