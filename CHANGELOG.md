...
- **`Financials.income_statement().to_dataframe()` returned an all-NaN column with a stray prior-year date on CAVA's Q1 fiscal 2026 10-Q** — `_select_quarterly_periods` bucketed duration periods as quarterly (80-100 days) or YTD (150-285 days), leaving a 101-149 day dead zone. CAVA's 111-day Q1 (and any 16-week-quarter filer on the 4-4-4-4 retail fiscal calendar) fell into that gap and was silently dropped; the parser then promoted a stray 83-day prior-year fragment as the "current quarter" — yielding an empty statement labelled with the wrong date. Period selection is now anchored on `filing.period_of_report`: durations ending at the report period are promoted to the current quarter/YTD slots regardless of length, and prior-year comparatives are seeded by matching the anchor's actual duration ±10 days. The bucket logic remains as fallback. Verified against CAVA (every Q1 since IPO was broken; Q2/Q3 unaffected) and Red Robin Gourmet Burgers; the dead zone affected 2 of 200 sampled 2026 10-Qs. ([#822](https://github.com/dgunning/edgartools/issues/822), reporter @mkdeak)
