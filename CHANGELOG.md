...
- **`FundSeries.get_filings()` handed back a sibling fund's portfolio.** It delegated straight to the fund company, and a trust files one report per series under a single CIK — so every series of a trust received the same interleaved list, and `filings[0]`, the idiom for "the latest one", belonged to whichever series filed most recently. Two Vanguard series (CIK 36405) both returned 337 filings topped by the same accession, which belonged to a third sibling: `VEXMX`, the Extended Market Index fund, which excludes the S&P 500 by construction, was handed the 500 Index fund's holdings. Nothing about the result looked wrong — real filings, from the right trust, parsing cleanly and summing to 100% — and it was not Vanguard-specific. `FundSeries` now resolves its own series through SEC browse-edgar, the path `Fund.get_filings(series_only=True)` has used since GH #888, and returns an empty `Filings` rather than falling back to the trust, because that fallback is the bug. Two series of one trust now return 29 and 28 filings, each topped by its own. Synthetic `ETF_*` series IDs, which are not SEC series and cannot be resolved, still use the trust.

  `Fund.get_portfolio()` and `Fund.get_latest_report()` were reachable by the same route and are fixed with it: both chained through `get_filings()` without `series_only`, so `Fund("VEXMX").get_latest_report()` returned a report belonging to `S000002846`. They now request their own series whenever the `Fund` names one. A `Fund` built from a multi-series trust CIK names no series and is unchanged.

  Two notes on the series path. `year` and `quarter` accept a list, as on the entity path — previously a list was untranslatable, silently dropped, and the caller received the series' entire unfiltered history; a pair that still cannot be expressed as date ranges now raises `ValueError` rather than answering with everything. And this path returns `Filings` rather than `EntityFilings`, so `report_date`, `acceptance_datetime` and `items` are not available on the returned filings — as has been true of `Fund.get_filings(series_only=True)` since GH #888.

  `Fund.get_filings()` without `series_only` is unchanged and still returns the whole trust: its default path reached the series entity, so it is now routed to the fund company explicitly — otherwise this fix would have silently made the documented default series-scoped and reduced `series_only=True` to a no-op. (GH #1143)
