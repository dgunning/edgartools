...
- **`Fund.get_filings(series_only=True)` now actually filters to the fund's series** — the series-aware path searched EFTS full text for the series ID, which returns nothing (SEC full-text search does not index NPORT series IDs), so it silently fell through to the unfiltered umbrella-trust delegation and returned a *sibling* series' data (e.g. `Fund("VCLT")` / series `S000026864` returned 444 trust filings, the first belonging to `S000026865`). Series filtering now resolves the series via SEC browse-edgar (series ID as the CIK parameter), which returns exactly that series' filings (VCLT now returns 26 NPORT filings, all `S000026864`). The requested form is pushed to browse-edgar server-side (`&type=`) so large funds return only the relevant filings in a single page instead of paging through their entire history — which SEC rate-limits/503s on deep pages, e.g. Vanguard's `VOO` (400+ filings) which now returns its 27 NPORT filings reliably. When a requested series has no matching filings, `series_only=True` returns an empty `Filings` rather than the unfiltered trust. (GH #888)
- **Form 4/5 `obj()` no longer crashes when a transaction has no `<transactionCoding>`** — filings with a `<nonDerivativeTransaction>` (or `<derivativeTransaction>`) that omits the `<transactionCoding>` block raised `AttributeError: 'DataFrame' object has no attribute 'Code'` during XML parsing, making the entire `Form4`/`Ownership` object unreachable (no owner, security, shares, price, or A/D data recoverable). The `Code`/`form`/`EquitySwap` columns are only populated for coded transactions, so a table where no transaction had coding produced a DataFrame missing the `Code` column that the downstream `df.Code.apply(...)` required. Those columns are now guaranteed to exist (defaulting to `None`) in both the non-derivative and derivative extractors, so uncoded rows degrade gracefully (`TransactionType` is `None`) instead of aborting the parse. Repro accessions: `0001133416-26-000010`, `0000897069-26-001304`, `0000912282-25-001299`. (GH #887)
- **Images (`<img>`) are no longer silently dropped from the new parser's markdown output** — in the `edgar.documents` pipeline, `<img>` elements were parsed into `ImageNode`s but then pruned by the postprocessor (which treated any childless node with no text `content` as empty), and the markdown renderer had no image branch, so charts that carry real content (e.g. a 10-K stock-performance graph) vanished from `Document.to_markdown()`. Image nodes are now preserved and rendered as `![alt](url)`, with a relative `src` resolved against the source document URL (`document.metadata.url`) when known so the link is a self-contained SEC-archive URL. The text extractor gains an opt-in `include_images` flag that emits an `[Image: <alt or filename>]` placeholder (default off, so clean text is unchanged). Note: `Filing.markdown()` still uses the legacy parser, which does not yet capture images — that path is tracked separately. (GH #886)
- **`get_revenue()` / `get_net_income()` / `get_operating_income()` no longer return a prior-year value on multi-duration 10-Qs** — the standardized income getters selected the period column positionally (`period_columns[period_offset]`), assuming index 0 was the most recent period. `RenderedStatement.to_dataframe()` does not order its columns by recency, so on a 10-Q that reports both 3-month and YTD columns for the current and comparative year the getters silently returned the wrong period: IonQ's FY2025 Q2 10-Q (`0000950170-25-104066`) gave `get_revenue()` = `11,381,000` (FY2024 Q2 comparative) instead of `20,694,000` (FY2025 Q2), and the FY2025 Q3 10-Q gave `12,400,000` instead of `39,866,000`. Period columns are now ordered from each period's metadata (current reporting period first — the shortest duration at the latest end date — then that same-duration series backwards in time, then other durations, then instant periods), so `period_offset` walks a single coherent series. Annual filings (latest fiscal year first) and balance-sheet getters are unaffected. (GH #885)
