...
- **`EntityFacts.get_concept()` no longer returns years-stale data for companies that switched GAAP tags** — `get_concept("capex")` resolved a concept's synonym group in priority order and returned on the *first* synonym holding any fact, so a company that moved a concept to a different tag (e.g. NVIDIA and Amazon switching capex from `PaymentsToAcquirePropertyPlantAndEquipment` to `PaymentsToAcquireProductiveAssets`) got the most-recent value *under the abandoned tag* — NVDA a 2020 figure (~4.7× understated), AMZN a 2017 figure (~6× understated) — with no error and no staleness signal, even though the current tag was present in the group. When no explicit period is requested, `get_concept()` now picks the most recent fact *across all synonyms* (by filing date then period end, ties broken by priority), mirroring the intra-tag recency `get_fact()` already applies; an explicit `period=` still resolves first-match-by-priority. The `return_metadata=True` dict now also carries the resolved `period`, `period_end`, and `filing_date` so a stale pick is visible. Fixes all 60 synonym-group concepts, not just capex. (edgartools-2920, GH #892)
- **`EntityFacts.get_ttm_revenue()` / `get_ttm_net_income()` no longer return badly stale values for companies that switched revenue tags** — the companion of the `get_concept()` fix above. These helpers tried candidate concepts in a fixed order and returned the *first* that yielded ≥4 quarters, so a company that migrated its revenue tag (NVIDIA and GOOG moved from `RevenueFromContractWithCustomerExcludingAssessedTax` to `Revenues`) stayed on the abandoned tag and summed *its* most-recent quarters: NVDA reported `$10.9B` (its real FY2020 total, ~25× understated) with `periods=[(2020,'FY')]×4` and no warning; GOOG lagged a full year — both silent, a plausible float with no `has_gaps` or `warning`. They now evaluate every candidate concept and return the one whose TTM window ends most recently (tie-broken by priority), moving both onto the current `Revenues` tag (NVDA `$253.5B`, GOOG `$422.5B`, AMZN unchanged at `$742.8B`). As defense-in-depth, `TTMCalculator` now sets a new `TTMMetric.is_stale` flag (and a warning) when the newest quarter lags the reference date — the caller's `as_of`, or today — by more than one reporting cycle, so genuinely stale data across *all* candidates stays programmatically detectable. (edgartools-c31n, GH #893)
- **10-K sections detected by the pattern extractor now carry `.item`/`.part` metadata** — on small-cap 10-Ks whose section headers split the bare `Item 7.` fragment from its `Management's Discussion and Analysis` title across separate HTML nodes (e.g. Nathan's Famous `0001437749-26-019923`), every semantic-keyed section (`mda`, `business`, `risk_factors`, …) came back with `section.item = None`/`section.part = None`, silently dropping them from any consumer that keys on `.item`. Only literally-named keys like `part_iii_item_12` resolved. `FormSchema.resolve_section_key()` now recovers the item from the schema's own title vocabulary (`Item 7 - MD&A` → `7`) and the part from the item→part ranges, so `mda` reports Part II / Item 7, `business` reports Part I / Item 1, and so on. Heading-detected sections resolve `part`/`item` the same way. (edgartools-8sq1, GH #891)
