...
- **497K fee tables reported the net expense ratio as the fee waiver, and never populated `net_expenses`** — for every 497K whose fee table uses the standard SEC wording "Total Annual Fund Operating Expenses After Fee Waiver and Reimbursement", `ShareClassFees.fee_waiver` held the net expense ratio and `net_expenses` was `None`: ProShares UltraPro QQQ came back with `total=0.97, waiver=0.84` when 0.84 is the net ratio and the waiver is 0.13. The label chain in `_extract_operating_expenses` guarded its "total annual" branch against the word "after" but not its "fee waiver" branch, so the net row matched as a waiver, overwrote the real waiver value parsed one row earlier, and left the `net_expenses` branch unreachable. Net expenses are now matched first — including the "Net Annual Fund Operating Expenses" wording, which no branch recognized — and the waiver is normalized to a signed reduction so `total_annual_expenses + fee_waiver == net_expenses` holds whether the filer wrote it parenthesized ("(0.19)%") or bare ("0.03%"). Across a 30-filing sample from 2024Q3, all 14 share classes reporting both values now reconcile; previously 15 of 15 were wrong. (GH #912)
- **497K fee values were corrupted by footnote markers sharing the value cell** — filers routinely put the footnote reference in the cell with the number, and `_parse_percentage` collapsed the whitespace between them, gluing the marker on: John Hancock's management fee `"0.67 1"` parsed as `0.671`, its total `"1.10 2"` as `1.102`, and `"0.46 3 4"` as `0.4634`. Every percentage field was affected — management fees, other expenses, totals, waivers and net ratios — not just the waiver. A second shape lost the value outright: for `"(0.37%)"` (GMO, Goldman Sachs, Columbia), stripping the `%` and everything after it ate the closing bracket, so the negative-value test failed and the waiver came back `None`. The parser now unwraps a parenthesised negative before touching the `%` sign and takes the leading numeric token only when what follows it looks like a footnote marker — digits or a reference symbol, optionally bracketed. That also accepts `"0.67*"` and `"0.67(1)"`, which previously failed to parse at all, while cells that merely begin with digits stay `None`: dates (`"2/1/2010"`, `"9-13-2017"`), labels (`"12b-1 Distribution Fee"`), and period headers (`"1 Year"`, `"5 Years"`). Those last two groups fixes a second, quieter bug — the best/worst-quarter extractor keeps the last cell that parses, so GMO Trust's `"4Q 2023"` and `"1Q 2022"` date cells parsed as 4 and 1 and overwrote the real returns of 6.02% and −6.88%. Across 135 sampled filings from three quarters, every share class reporting both a waiver and a net ratio now satisfies `total + waiver == net`; 17 did not before, and a cell-by-cell diff over 8,783 fee, performance and quarter cells shows no value changing except in these intended directions. (GH #912)
- **TTM Earnings Per Share returned a stale trailing year, and lost precision on screen** — `company.income_statement(period='ttm')` reported GOOGL basic EPS of 9.45 for Q2 2026 when the correct trailing-twelve-month figure is 20.16; the same period returned 20.16 at `periods=4` but 9.45 at `periods=8`, and three periods came back `NaN`. The TTM EPS path labelled each window with the as-of fact's SEC-tagged `fiscal_year`, but the SEC tags a quarter re-filed as a comparative with the *filing's* fiscal year — GOOGL's Q2 2025 carries `fy=2026` — so two windows were labelled "Q2 2026" and the older one silently overwrote the current one in the period dict. Only EPS was affected: every other concept goes through `calculate_ttm_trend()`, which has derived its labels from `period_end` plus the fiscal-year-end month since GH #793. The EPS path now does the same, so results no longer depend on the requested period count and non-calendar filers (AAPL, MSFT, NVDA) label correctly. A colliding label now keeps the most recent window instead of the oldest. Separately, the console renderer formatted any value under $1M as whole dollars, printing EPS of 20.16 as "$20" while `to_dataframe()` kept full precision; per-share amounts now render with 2 decimals, and monetary rows keep their B/M formatting. (GH #910)
- **`series_only=True` now filters when the `Fund` was built from a series or class ID** — `Fund("S000026864").get_filings(form=["N-PORT", "NPORT-P"], series_only=True)` returned the umbrella trust's 444 filings, whose latest entry belongs to a sibling series, while `Fund("VCLT")` for the same fund correctly returned 26. `Fund.__init__` populated `_target_series_id` only on the ticker resolution path, so for `Fund("S000...")` and `Fund("C000...")` it stayed `None`, the series-aware branch in `get_filings()` was skipped entirely, and the call fell through to trust-wide entity delegation — the exact harm fixed for tickers in #888. The target series is now backfilled from the resolved hierarchy, so ticker, series ID and class ID for one fund return the identical filing set. (GH #909)
- **Quarterized cash flow no longer silently drops every negative quarter** — `TTMCalculator.quarterize()` returned only Q1 for concepts like `NetCashProvidedByUsedInInvestingActivities` (12 quarters instead of ~48 for GOOGL), even with every YTD and FY input present. Q1 is the one quarter needing no subtraction, so all derived quarters were being discarded: `_is_positive_concept` substring-matched `'cash'` from its must-be-positive keyword list, so any cash flow — investing and financing are negative in most periods — was treated as a data-quality error and skipped by the Q2/Q3/Q4 derivation guards. The keyword protected nothing, since the cash *balance* concepts it appeared to target are instant facts that `_is_additive_concept` already rejects earlier. Operating cash flow was unaffected only because it is usually positive. The same defect hit `IncreaseDecreaseIn*` working-capital lines, which matched `'receivable'`/`'revenue'`/`'asset'`; period-over-period movements are now treated as signed too. GOOGL FY2024 investing now quarterizes to −8.564B / −2.781B / −18.011B / −16.180B, reconciling exactly to the reported −45.536B. (GH #907)
- **10-K items no longer overflow to end-of-document when the TOC anchors only a subset of items** — Coeur Mining's FY2025 10-K returned `TenK['Item 7']` as 257K chars running from the Item 7 header to the director signatures (Items 7A/8/9A/10/11 embedded inside), and Item 1B as 317K chars for a one-line "None." stub. Three gaps combined: the body-header scan's bold check missed headers whose font weight lives on child spans (only the *title* span bold — "Item 7A." at weight 400); body-header recovery ran only below the 10-K item floor and replaced the map wholesale, so a TOC that anchored exactly 8 canonical items was never repaired; and adjacent items sharing one anchor div (Coeur's 1B/1C) produced an empty span the slicer treated as unbounded. The bold check now accepts split-span headers, body-header recovery union-merges the items the TOC missed (TOC wins conflicts), and end boundaries skip same-anchor sections. A new successor-header guardrail also flags any remaining TOC item section embedding a line-anchored header of a later, undetected item — the over-capture class size bands can't see (Item 7's band is generous; Item 1B has no band). The body-header map is rejected outright when its items collapse onto shared stale anchor ids (filers with no per-item anchor divs, e.g. Nathan's Famous, stay on their pattern-detection path), and the guardrail's completeness pre-gate treats the optional Item 16 as no gap, so filings that omit it (JPM, XOM) pay no extraction cost. Standard filers (AAPL, Citigroup) parse byte-identically. (GH #904)
