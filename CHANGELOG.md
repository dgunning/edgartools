...
- **`Company.get_facts()` re-downloaded companyfacts on every call, and the 30s `/submissions` TTL never took effect.** `_get_cache_rules()` filed every rule under `SEC_BASE_URL`, but httpxthrottlecache matches the request *host* against that key before the path, and `re.match(r'.*www\.sec\.gov', 'data.sec.gov')` is `None`. Both endpoints come from `SEC_DATA_URL`, so a fresh process logs `No patterns matched data.sec.gov` and goes to the network — including the 600s → 30s reduction made for GH #471. Cache keys now match the request host exactly, so a rule written for one host can no longer answer for another. (GH #989)
