...
- **10-K Item 7A silently duplicating Item 7** — REGIONS FINANCIAL CORP's FY2021 10-K (`0001281761-22-000016`) returned Item 7's 194K-char MD&A body for `obj['Item 7A']` (`obj['Item 7A'] == obj['Item 7']`). This filing's linked TOC carries only page numbers, so the generic parser resolved each item to the anchor of the page it starts on — Item 7 (MD&A) and Item 7A both begin on page 41 and collided on that single anchor, slicing to the identical span with no warning. The TOC parse now detects two distinct item keys resolving to one anchor and re-resolves the displaced item from the body-header scan (which carries each item's own heading behind its own anchor): Item 7 keeps the shared page anchor and Item 7A resolves to its real two-line incorporation-by-reference stub. When the body scan can't separate them, the collision is logged rather than left silent. Filers whose items each anchor distinctly parse unchanged. (GH #920)
- **Workiva 10-K items mis-anchored or dropped when TOC label and title links disagree** — Tesla's FY2023 10-K returned 145K chars for `TenK['Item 3']` (the legal-proceedings body plus everything through Part II Item 5) because its Workiva TOC gives the "Item N." label link a different, broken href than the title link. The parser took the first href group that parsed to an item, so labels pointing at nonexistent targets silently dropped Items 1B/4/5 (titles like "Mine Safety Disclosures" aren't in the keyword vocabulary), and labels pointing at wrong-but-existing targets mis-anchored Items 1/1A (and truncated Item 8 to 2.5K chars via the knock-on boundary shift). Each TOC row is now resolved as a whole: when a row names a single item, the anchor is chosen across all of the row's href groups, preferring a target that exists and sits next to the item's own heading. On the Tesla filing the Workiva parse now matches the generic parse (24 sections, identical anchors); healthy Workiva TOCs, where a row's links share one href, parse unchanged. (GH #915)
- **`edgartools[ai]` no longer installs an incompatible `mcp`** — mcp 2.0.0 removed the decorator-based `Server` API (`@app.list_tools()` and friends) that `edgar/ai/mcp/server.py` binds at import time, so an unpinned resolve made `uvx --from "edgartools[ai]" edgartools-mcp` fail with `AttributeError: 'Server' object has no attribute 'list_tools'` before it could start. The dependency is capped at `mcp>=1.12.3,<2.0.0`; the cap lifts once the server is ported to the 2.0 handler API. (GH #917, beads edgartools-lcd3)
- **`FilingSGML.text()` renders ownership forms instead of returning raw XML** — for Forms 3/4/5 (and amendments) the primary document is `<ownershipDocument>` XML, which failed the is-it-HTML check and came back verbatim: tag soup instead of a readable Form 4, on the highest-volume SEC form type. Ownership XML is now detected by its root element and rendered through the same `Ownership.to_html()` the data objects use, so `FilingSGML.text()` and `Filing.text()` agree byte-for-byte on modern ownership filings. The new pipeline lives in `edgar/sgml/text_extraction.py`; `Filing.text()`/`Filing.html()` are deliberately untouched (their contracts are load-bearing — see the d216a934 revert), so declaration-less 2004-era ownership XML still renders poorly through `Filing.text()` and correctly through `FilingSGML.text()`. (beads edgartools-rck1)
- **Unterminated HTML comments no longer crash text extraction** — 1990s-era filings open with `<!--DOCTYPE HTML PUBLIC ...>` (a typo for `<!DOCTYPE`) or a comment that simply never closes; lxml treats the stray `<!--` as running to end of input, the parse tree comes back empty, and both `Filing.text()` and `FilingSGML.text()` raised `HTMLParsingError: Document is empty` (0001034670-01-500017). Unterminated comments are now closed at the end of their line before lxml parses — normal comments are untouched, verified byte-identical output across a 44-filing corpus — and `FilingSGML.text()` additionally degrades to tag-stripped text rather than raising if the parser still fails. (beads edgartools-j8bs)
- **`FilingSGML.text()` no longer returns mojibake for PDF-only filings** — UPLOAD comment letters with a scanned-PDF primary decoded the binary as UTF-8 and returned pages of replacement characters that poisoned search and markdown downstream. Binary primaries are now detected before any decode; `text()` returns the SEC's own TEXT-EXTRACT plain-text sibling when the submission has one, and otherwise `None` — a truthful "no text here". `Filing.text()`'s UPLOAD behavior is unchanged (pinned by its correspondence contract). (beads edgartools-e0hr)
- **8-K `.items` no longer drops items** — two causes fixed. 2005-era plain-text filings glue an item header onto a horizontal rule with no line break (`------Item 4.02 Non-Reliance...`), which the line-start-anchored fallback regex missed: GMAC 0000040729-05-000026 reported only `['Item 8.01']` instead of `['Item 4.02', 'Item 8.01']`. And when the HTML-based strategies returned a partial set, the text strategy was never consulted at all (Cimarex 0001047469-05-006981 lost Item 9.01). Item headers are now also recognized after a 5+ character rule — with the same anchor used for content retrieval, so `eightk['Item 4.02']` returns the right section — and all three detection strategies are unioned. Mid-sentence references ("as described in Item 8.01") still don't match; validated on 66 modern 8-Ks with zero spurious items, and the union also recovers genuinely missed items on modern filings (an ExxonMobil 8-K regained its Item 7.01). (beads edgartools-l6cl)
- **SGML header parser no longer leaks a stale subheader across sections** — headers from the 1999–2005 era that follow a nested `FILER`/`MAIL ADDRESS` block with a flat top-level section made the parser carry the old subheader into the new section, emitting `Subheader 'MAIL ADDRESS' not found in header 'COMPANY DATA'` warnings and silently dropping the section's values; on 1990s headers with `FORMER COMPANY` entries the same leak crashed with an uncaught `KeyError`. The subheader now resets at each new top-level section, flat key/value lines are captured, and the warning (still possible on genuinely malformed headers) includes the accession number so bulk-processing logs are traceable. (beads edgartools-nzfp)
- **Filings from 1993 Q1–1994 Q2 are now reachable** — `available_quarters()` hardcoded EDGAR's start as 1994 Q3, so `get_by_accession_number()` and `get_filings()` structurally rejected earlier periods even though SEC's full-index serves every quarter back to 1993 Q1 (verified; 1992 and earlier return errors — EDGAR's electronic pilot began in 1993). The boundary, the "Valid range" warning, and the open-ended `filing_date` range default (previously `1994-06-01`) now all start at 1993 Q1: `get_by_accession_number("0000003673-94-000052")` returns the actual 1994 Q2 filing instead of `None`. (beads edgartools-g00j)
