...
class TestHelperFunctions:
    ...
    def test_is_requesting_current_filings_open_range_to_today(self):
        """Test detection with open-ended range to today (recent start)"""
        three_days_ago = date.today() - timedelta(days=3)
        date_range = f"{three_days_ago.isoformat()}:"  # Open-ended to today
        assert _is_requesting_current_filings(date_range) is True

    def test_is_requesting_current_filings_historical_date(self):
        """Test that old historical dates (>5 days) don't trigger warning"""
        one_week_ago = date.today() - timedelta(days=7)
        assert _is_requesting_current_filings(one_week_ago.isoformat()) is False
...
class TestWarningMessages:
    ...
    def test_warn_use_current_filings_with_date(self, mock_print_warning):
        """Test warning message includes date information"""
        three_days_ago = date.today() - timedelta(days=3)
        _warn_use_current_filings("Test reason", three_days_ago)

        # Verify print_warning was called
        assert mock_print_warning.called
        warning_message = mock_print_warning.call_args[0][0]

        # Check message contents - new format is "Data through {date}."
        assert "Data through" in warning_message
        assert str(three_days_ago) in warning_message

    @patch('edgar._filings.print_warning')
    def test_warn_use_current_filings_without_date(self, mock_print_warning):
...
class TestFilingsLatestWarning:
    ...
    @patch('edgar._filings.print_warning')
    def test_latest_warns_on_stale_data(self, mock_print_warning):
    ...
    @patch('edgar._filings.print_warning')
    def test_latest_no_warning_on_recent_data(self, mock_print_warning):
        ...
        assert not mock_print_warning.called
...
class TestFilingsFilterWarning:
    ...
    @patch('edgar._filings.print_warning')
    def test_filter_warns_when_requesting_today_with_stale_data(self, mock_print_warning):
    ...
    @patch('edgar._filings.print_warning')
    def test_filter_no_warning_for_historical_date(self, mock_print_warning):
        """Test that filter() doesn't warn for old historical queries (>5 days)"""
        # Create mock filing data from 2 weeks ago
        two_weeks_ago = datetime.now() - timedelta(days=14)
        two_weeks_ago_str = (date.today() - timedelta(days=14)).isoformat()
        ...
        result = filings.filter(filing_date=two_weeks_ago_str)

        # Verify NO warning (this is clearly an old historical query, >5 days)
        assert not mock_print_warning.called
