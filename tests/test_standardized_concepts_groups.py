...
def revenue_across_tech_companies_test(company):
...
@pytest.mark.network
@pytest.mark.slow
@on_nasdaq_top20(max_failures=3)
def core_metrics_nasdaq_top20_test(company):
...
def complete_financials_mega_cap_test(company):
...
@pytest.mark.network
@pytest.mark.slow
@run_on_company_group("faang", max_failures=0)  # FAANG should all work
def faang_standardization_test(company):
...
@pytest.mark.network
@pytest.mark.slow
@on_diverse_sample(max_failures=6)
def concept_mapping_coverage_test(company):
...
@pytest.mark.network
@pytest.mark.slow
@run_on_company_group("dow_sample", max_failures=3)
def period_specific_access_test(company):
...
@pytest.mark.network
@pytest.mark.slow
def test_user_workflow_example():
    ...
    @run_on_company_group(user_companies, max_failures=0)
...
@pytest.mark.network
@pytest.mark.slow
@run_on_company_group("diverse_sample", max_failures=8)
def missing_concepts_return_none_test(company):
...
@pytest.mark.network
@pytest.mark.slow
@run_on_company_group("tech_giants", max_failures=3)
def unit_consistency_test(company):
