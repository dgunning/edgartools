...
This file contains changelog entries for releases **prior to 5.20.0** (anything older than ~6 months from the current release).

For current and recent releases, see [CHANGELOG.md](CHANGELOG.md).

This archive is read-only and is not updated except when older entries are rolled in from the main changelog.
