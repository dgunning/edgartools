...
def test_calculate_ttm_from_quarters():
    facts = [
        make_fact(
...
def test_quarterize_derives_q2_q3_q4():
    facts = [
        make_fact(
            concept="us-gaap:Revenues",
            value=100,
...
def test_detect_splits_filters_long_lag():
    split = make_fact(
...
def test_apply_split_adjustments_eps_and_shares():
    ...
    assert adjusted[0].numeric_value == 5
    assert adjusted[1].numeric_value == 200


def test_derive_q4_eps_from_net_income_and_shares():
    net_income_facts = [
        make_fact(
